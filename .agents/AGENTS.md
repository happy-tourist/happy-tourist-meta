...
| [`work-with-styles`](skills/client/work-with-styles/SKILL.md) | Quasar Dark + header, variables, muted chrome, board CSS |
...
| [`server-work-with-test`](skills/server/server-work-with-test/SKILL.md) | mocha + `@colyseus/testing` (rooms + `POST /api/theme`) |
...
| [`work-with-routes`](skills/server/work-with-routes/SKILL.md) | HTTP `createEndpoint` / thin routes (incl. `POST /api/theme`) |
