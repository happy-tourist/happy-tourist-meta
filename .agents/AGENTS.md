...
| [`client-locate-change-points`](skills/client/client-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. Google auth, support/content/admin pages + `stores/support` / `stores/content` dual submit + D1′/D5′/author delete / collection-only Edit / staff redirect; Vitest harness) |
...
| [`work-with-routes`](skills/server/work-with-routes/SKILL.md) | HTTP `createEndpoint` / thin routes (incl. `GET|POST /api/theme`, auth email JSON + display-name/change-password, `/api/support/*` create-ack + staff query filters, `/api/content/*` dual submit + D1′/author delete + staff nested hub, `/api/admin/*` non-anonymous + `emailVerified`) |
