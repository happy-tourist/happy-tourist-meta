...
| [`client-work-with-structure`](skills/client/client-work-with-structure/SKILL.md) | pages / components / `src/lib` helpers / boot / stores placement (theme shell + Game leave/status in App; support store; password policy helpers) |
...
| [`work-with-localization`](skills/client/work-with-localization/SKILL.md) | vue-i18n boot; auth-email RU + spam hint (forgot/confirm/reset SPA); password policy / strength / displayName / change-password keys; `support.*` topics/statuses/filters/emailUnverified/errors/roles; `game.say` / ready / leave / finish / returnAffordance / pushAffordance / timer+steps end / steps/peeks / endTurn / peek / solo-peeks∞ / grille+catapult density + rescue/return/all-jail keys |
