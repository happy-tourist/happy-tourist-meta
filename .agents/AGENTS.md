...
| [`client-locate-change-points`](skills/client/client-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. Google auth, support/content/admin pages + `stores/content` dual submit + my-moderation / three-phase marks / D1′/D5′/author delete / live Edit in-collection + trash/`inCollection` / staff reject-stays + approve-from-rejected + hub `tasksOnly`; Vitest harness) |
...
| [`server-locate-change-points`](skills/server/server-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. `config/auth`, `lib/passwordPolicy`, `lib/support`, `lib/content` dual answers\|tasks + D1′/D5′/author delete/my-moderation/reject-stays/approve-from-rejected/staff queue tasks-only + hub `tasksOnly`/`answersActionsAvailable`, support/content/admin + auth profile HTTP) |
...
| [`work-with-routes`](skills/server/work-with-routes/SKILL.md) | HTTP `createEndpoint` / thin routes (incl. `GET|POST /api/theme`, auth email JSON + display-name/change-password, `/api/support/*` create-ack + staff query filters, `/api/content/*` dual submit + live open author ids + D1′/D5′ on open + author delete + `my-moderation` + reject-stays + approve-from-rejected (`not_approvable`) + staff queue tasks-only + hub `tasksOnly`/`answersActionsAvailable`, `/api/admin/*` non-anonymous + `emailVerified`) |
...
| [`work-with-database`](skills/server/work-with-database/SKILL.md) | GameDatabase, users schema (incl. nullable `theme`, `emailVerified`, `htRole`) + support_tickets/messages + content_* pack tables (`type` answers\|tasks, open pending\|rejected, snapshots/`tasksDirty`/`needsModeration` + three-phase + D1′/author delete, `ensureContentTables`) |
