...
| [`work-with-pages`](skills/client/work-with-pages/SKILL.md) | Routes + App shell; Lobby «Наборы»→`content-catalog`; unified packs filters/star; collection redirect; author re-edit; staff take; MapsList filters; GamePage |
| [`work-with-stores`](skills/client/work-with-stores/SKILL.md) | Pinia domains; `content` unified list + favorites + author re-edit + staff take ([content.md](skills/client/work-with-stores/content.md)); `maps` statuses/filters ([maps.md](skills/client/work-with-stores/maps.md)) |
...
| [`work-with-localization`](skills/client/work-with-localization/SKILL.md) | vue-i18n; `content.*` filters/favorites/take/statuses + errors `author_request_open`/`moderation_taken`; `maps.*` filters/statuses; support/auth/game |
| [`work-with-lobby`](skills/client/work-with-lobby/SKILL.md) | Live LobbyRoom; header Support + «Наборы»→`content-catalog` + «Карты»→`content-maps` |
...
| [`work-with-test`](skills/client/work-with-test/SKILL.md) | Vitest; ContentUnifiedList/favorites/AuthorEditTake + maps + soft-unpublish FollowUp/ModerationUx; topic files |
...
| [`server-locate-change-points`](skills/server/server-locate-change-points/SKILL.md) | Где править (content unified list/favorites/author re-edit/staff take; maps statuses; default grants retired SC-PACK-170) |
...
| [`server-work-with-structure`](skills/server/server-work-with-structure/SKILL.md) | rooms/schema/db/config/lib (content unified list + favorites + author re-edit + staff take; maps; default grants no-op; support/mailer) |
| [`server-work-with-test`](skills/server/server-work-with-test/SKILL.md) | mocha; zz-contentPacks unify/favorites/author-edit/take; zz-contentMaps; zz-defaultContentPacks SC-PACK-170; rooms/auth/support |
...
| [`work-with-routes`](skills/server/work-with-routes/SKILL.md) | HTTP thin routes; `/api/content/*` unified list + favorite + author draft/lock + staff take/release + maps; collection legacy; grants retired |
