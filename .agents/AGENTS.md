...
| [`client-locate-change-points`](skills/client/client-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. Google auth, support/content/admin pages + `stores/support` / `stores/content` dual submit + D1′/D5′/author delete / live Edit in-collection + trash/`inCollection` / hide Edit if `blocked` / staff queue tasks-only + hub `tasksOnly`; Vitest harness) |
...
| [`server-locate-change-points`](skills/server/server-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. `config/auth`, `lib/passwordPolicy`, `lib/support`, `lib/content` dual answers\|tasks + D1′/D5′/author delete/staff queue tasks-only + hub `tasksOnly`/`answersActionsAvailable`, support/content/admin + auth profile HTTP) |
...
| [`server-work-with-structure`](skills/server/server-work-with-structure/SKILL.md) | rooms / schema / db / config / lib (mailer + support + content dual answers\|tasks + live `inCollection`/pending ids + D1′/author delete + staff queue tasks-only + hub `tasksOnly`/`answersActionsAvailable` + passwordPolicy) / app.config placement |
| [`server-work-with-test`](skills/server/server-work-with-test/SKILL.md) | mocha + `@colyseus/testing` (SC-PIECE + SC-START + SC-MOVE peeks∞/solo become-current +1 / step-loss/holes/grilles trap-rescue-push-return/catapult paced+deferred+idle re-eval SC-MOVE-78…93/leave-clear + SC-BOARD + SC-FINISH + SC-SAY + lobby density 12/22/35% + `GET|POST /api/theme` + auth email/policy + `zz-authProfile` SC-PROFILE + `test/support.test.ts` SC-SUP/SC-ROLE incl. create-ack/filters/no self-reply mail/role `emailVerified` + `zz-contentPacks.test.ts` SC-PACK dual answers|tasks + live `inCollection` (SC-PACK-61…64) + D1′/author delete/staff queue tasks-only + hub `tasksOnly`) |
...
| [`work-with-database`](skills/server/work-with-database/SKILL.md) | GameDatabase, users schema (incl. nullable `theme`, `emailVerified`, `htRole`) + support_tickets/messages + content_* pack tables (`type` answers\|tasks, snapshots/`tasksDirty`/`needsModeration` + D1′/author delete, `ensureContentTables`) |
