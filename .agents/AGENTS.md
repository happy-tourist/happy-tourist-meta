...
| [`work-with-game-board`](skills/client/work-with-game-board/SKILL.md) | Board + top opponents / spectator presence + seated `.game-hud` strip (row/2×2; no chip/`q-menu`) + grille (`GRILLE_ANIM_MS=1000`) + trap/rescue/push + return strip icon (no modal) + dual rings + budgets above + end-turn dock + peek + holes + say top↓/own↑ + nearest-center finish + return anim; leave/status in App |
