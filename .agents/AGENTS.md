...
| [`work-with-pages`](skills/client/work-with-pages/SKILL.md) | Routes + guards login/lobby/game; App header (theme + Game leave/status); Lobby create grilleDensity; GamePage bottom HUD / compact chip+`q-menu` / budgets/peek / holes / grilles trap-rescue-return-in-menu / dual end + all-jail modals overview |
...
| [`work-with-styles`](skills/client/work-with-styles/SKILL.md) | Quasar Dark + GET/POST `/api/theme`, header, muted chrome, tourist board (holes + grille overlays / `--grille-anim-ms` 1000) + bottom HUD chip/menu / budgets/peek CSS |
| [`work-with-localization`](skills/client/work-with-localization/SKILL.md) | vue-i18n boot; `game.say` / ready / leave / finish / `touristChipAria` / timer+steps end / steps/peeks / endTurn / peek / solo-peeks∞ / grille density + rescue/return/all-jail keys |
...
| [`work-with-game-board`](skills/client/work-with-game-board/SKILL.md) | Board + bottom `.game-hud` (no top-row) + compact 2×2 chip + upward `q-menu` picker + grille overlay/chrome (`GRILLE_ANIM_MS=1000`) + trap/rescue/return-in-menu + dual rings + own steps/peeks + end-turn + peek eye/modals + holes not landable + corner affordances + bubbles always up + place/timer/solo/all-jail modals + move/ready/say; leave/status in App |
