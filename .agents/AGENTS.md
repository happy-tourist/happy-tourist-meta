...
| [`server-locate-change-points`](skills/server/server-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. `config/auth`, `lib/passwordPolicy`, `lib/support`, `lib/content` dual answers\|tasks + statuses/D5′/staff nested, support/content/admin + auth profile HTTP) |
...
| [`work-with-routes`](skills/server/work-with-routes/SKILL.md) | HTTP `createEndpoint` / thin routes (incl. `GET|POST /api/theme`, auth email JSON + display-name/change-password, `/api/support/*` create-ack + staff query filters, `/api/content/*` dual submit + statuses/D5′ + staff nested hub, `/api/admin/*` non-anonymous + `emailVerified`) |
...
| [`work-with-database`](skills/server/work-with-database/SKILL.md) | GameDatabase, users schema (incl. nullable `theme`, `emailVerified`, `htRole`) + support_tickets/messages + content_* pack tables (`type` answers\|tasks, snapshots/`tasksDirty`/`needsModeration`, `ensureContentTables`) |
