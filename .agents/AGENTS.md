...
| [`client-locate-change-points`](skills/client/client-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. Google auth, support/admin pages + `stores/support`) |
...
| [`client-work-with-auth`](skills/client/client-work-with-auth/SKILL.md) | Colyseus Auth (email/anonymous/Google), SPA confirm/reset + JSON, forgot/cabinet/`emailVerified`, userdata `role` / `isStaff` / `isAdmin`, once-per-session verify reminder, `onChange`, route guards (`requiresStaff`/`requiresAdmin`) |
| [`client-work-with-errors`](skills/client/client-work-with-errors/SKILL.md) | Store `error` + `q-banner` (pages + App theme; auth/game/support), room `onError`; soft-drop rejoin gated by store `consentedLeaving` |
| [`client-work-with-structure`](skills/client/client-work-with-structure/SKILL.md) | pages / components / boot / stores placement (theme shell + Game leave/status in App; support store) |
| [`work-with-forms`](skills/client/work-with-forms/SKILL.md) | LoginPage / ForgotPasswordPage / ResetPasswordPage / Support create+reply `q-form` (`lazy-rules` + clear → `nextTick` → `resetValidation`) / guest + Google buttons |
| [`work-with-pages`](skills/client/work-with-pages/SKILL.md) | Routes + guards login/forgot/confirm/reset/account/lobby/support/admin/game; App header (theme + verify reminder + Game leave/status); Lobby create grilleDensity+catapultDensity + Support link; staff filters; single ticket Close (`canClose`); admin `emailVerified` badge; GamePage top opponents + seated HUD strip / budgets beside / end-turn icon on avatar / return strip icon (no modal) / push icons / nearest center / catapult land→overlay→fling + deferred grille + board-busy / dual end + all-jail overview |
| [`work-with-stores`](skills/client/work-with-stores/SKILL.md) | Pinia `auth` (forgot/confirm/reset/change-email + `role`/`isStaff`/`isAdmin`) / `theme` / `game` (incl. peeks∞ / finite steps / peek / rescue/push/return / grilleDensity+catapultDensity create / reveal keys / D13 atomic `$patch` seats+revealing / end-turn / holes) / `support` (tickets/staff/admin HTTP; `setUserRole` merge keeps `emailVerified`) |
| [`work-with-styles`](skills/client/work-with-styles/SKILL.md) | Quasar Dark + GET/POST `/api/theme`, header, muted chrome, tourist board (holes + grille overlays / `--grille-anim-ms` 1000) + catapult reveal/broken-hold CSS + top presence + seated strip HUD / budgets beside / end-turn icon / peek-rescue-push top-center / say top↓ own↑ CSS |
| [`work-with-localization`](skills/client/work-with-localization/SKILL.md) | vue-i18n boot; auth-email RU + spam hint (forgot/confirm/reset SPA); `support.*` topics/statuses/filters/emailUnverified/errors/roles; `game.say` / ready / leave / finish / returnAffordance / pushAffordance / timer+steps end / steps/peeks / endTurn / peek / solo-peeks∞ / grille+catapult density + rescue/return/all-jail keys |
| [`work-with-lobby`](skills/client/work-with-lobby/SKILL.md) | Live LobbyRoom subscribe, create-with-maxSeats + grilleDensity + catapultDensity modal (12/22/35% seed each; no Play shortcut), quiet resubscribe |
...
| [`work-with-game-board`](skills/client/work-with-game-board/SKILL.md) | Board + top opponents / spectator presence + seated `.game-hud` strip (row/2×2; no chip/`q-menu`) + grille (`GRILLE_ANIM_MS=1000`; defer drop during catapult hops) + catapult land→overlay→fling (`CATAPULT_ANIM_MS=1000`, broken 300+300, spectator parity, D13 atomic mirror, **finish travel after vanish** even if finish sync late — SC-FINISH-20 / SC-BOARD-31, board-busy lock) + trap/rescue/push + return strip icon (no modal) + dual rings + budgets beside + end-turn icon right-center + peek/rescue/push top-center + holes + say top↓/own↑ + nearest-center finish + move/push finish travel (`lastKnown` seed, not `finishAnimFrom` at submit) + return anim; leave/status in App |
...
| [`server-locate-change-points`](skills/server/server-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. `config/auth`, `lib/support`, support/admin HTTP) |
| [`server-verify-code`](skills/server/server-verify-code/SKILL.md) | Проверка кода на соответствие skills + DRY/KISS/YAGNI |
| [`server-work-with-auth`](skills/server/server-work-with-auth/SKILL.md) | `@colyseus/auth`, Google OAuth, smtp.bz mailer, SPA mail links + JSON confirm/reset, `emailVerified`, `htRole`→userdata `role`, `BOOTSTRAP_ADMIN_IDS`, send-confirm / change-email, JWT `onAuth` |
...
| [`server-work-with-structure`](skills/server/server-work-with-structure/SKILL.md) | rooms / schema / db / config / lib (mailer + support) / app.config placement |
| [`server-work-with-test`](skills/server/server-work-with-test/SKILL.md) | mocha + `@colyseus/testing` (SC-PIECE + SC-START + SC-MOVE peeks∞/solo become-current +1 / step-loss/holes/grilles trap-rescue-push-return/catapult paced+deferred+idle re-eval SC-MOVE-78…93/leave-clear + SC-BOARD + SC-FINISH + SC-SAY + lobby density 12/22/35% + `GET|POST /api/theme` + auth email + `test/support.test.ts` SC-SUP/SC-ROLE incl. create-ack/filters/no self-reply mail/role `emailVerified`) |
...
| [`work-with-routes`](skills/server/work-with-routes/SKILL.md) | HTTP `createEndpoint` / thin routes (incl. `GET|POST /api/theme`, auth email JSON, `/api/support/*` create-ack + staff query filters, `/api/admin/*` non-anonymous + `emailVerified`) |
...
| [`work-with-database`](skills/server/work-with-database/SKILL.md) | GameDatabase, users schema (incl. nullable `theme`, `emailVerified`, `htRole`) + support_tickets/messages |
