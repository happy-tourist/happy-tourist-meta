...
| [`work-with-pages`](skills/client/work-with-pages/SKILL.md) | Routes + App shell (section nav / crumbs / session logout / Game leave); Lobby room-list only; unified packs filters/star; collection redirect; author re-edit; staff take; MapsList/MapEditor view vs paint |
| [`work-with-stores`](skills/client/work-with-stores/SKILL.md) | Pinia domains; `content` unified list + cancel→draft + set marks ([content.md](skills/client/work-with-stores/content.md)); `maps` statuses/cancel→draft/view ([maps.md](skills/client/work-with-stores/maps.md)) |
...
| [`work-with-test`](skills/client/work-with-test/SKILL.md) | Vitest; ContentUnifiedList/favorites/AuthorEditTake + maps + AppHeaderChrome + soft-unpublish FollowUp/ModerationUx; topic files |
...
| [`server-locate-change-points`](skills/server/server-locate-change-points/SKILL.md) | Где править (content unified list/favorites/author re-edit/staff take/cancel→draft/set marks; maps statuses; default grants retired SC-PACK-170) |
...
| [`server-work-with-structure`](skills/server/server-work-with-structure/SKILL.md) | rooms/schema/db/config/lib (content unified list + favorites + author re-edit + staff take + working≠live draft + set marks; maps; default grants no-op; support/mailer) |
| [`server-work-with-test`](skills/server/server-work-with-test/SKILL.md) | mocha; zz-contentPacks unify/favorites/author-edit/take/cancel→draft/set marks; zz-contentMaps; zz-defaultContentPacks SC-PACK-170; rooms/auth/support |
...
| [`work-with-routes`](skills/server/work-with-routes/SKILL.md) | HTTP thin routes; `/api/content/*` unified list (working≠live → draft) + favorite + author draft/lock + set `moderationStatus` + staff take/release + maps; collection legacy; grants retired |
