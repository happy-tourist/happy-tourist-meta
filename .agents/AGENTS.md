...
| [`client-work-with-structure`](skills/client/client-work-with-structure/SKILL.md) | pages / components / `src/lib` helpers / boot / stores placement (brand logo left; Game leave via logo + status in App; no page «В лобби»; support store; password policy helpers) |
...
| [`work-with-styles`](skills/client/work-with-styles/SKILL.md) | Quasar Dark + GET/POST `/api/theme`, header brand-logo CSS + theme toggle, muted chrome, tourist board (holes + grille overlays / `--grille-anim-ms` 1000) + catapult reveal/broken-hold CSS + top presence + seated strip HUD / budgets beside / end-turn icon / peek-rescue-push top-center / say top↓ own↑ CSS |
| [`work-with-localization`](skills/client/work-with-localization/SKILL.md) | vue-i18n boot; auth-email RU + spam hint (forgot/confirm/reset SPA); password policy / strength / displayName / change-password keys; `auth.backToLobby` aria-only for logo; `support.*` topics/statuses/filters/emailUnverified/errors/roles; `game.say` / ready / leave (logo aria) / finish / returnAffordance / pushAffordance / timer+steps end / steps/peeks / endTurn / peek / solo-peeks∞ / grille+catapult density + rescue/return/all-jail keys |
