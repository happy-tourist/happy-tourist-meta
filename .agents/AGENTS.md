...
| [`work-with-lobby`](skills/client/work-with-lobby/SKILL.md) | Live LobbyRoom subscribe; create map+pack+taskSets + densities (maxSeats from map.players); map preview + pack/set labels; join busy-lock; quiet resubscribe; Packs/Maps/Support/Модерация/logout in App header — not LobbyPage |
