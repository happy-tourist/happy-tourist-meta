...
| [`client-locate-change-points`](skills/client/client-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. Google auth, support/content/admin pages + `stores/content` dual submit + my-moderation / three-phase marks / cascade outline + slot chips / draftStale+rebase / staff unpublish/republish / D1′/D5′/author delete / live Edit in-collection + trash/`inCollection` / staff reject-stays + approve-from-rejected + hub `tasksOnly`; Vitest harness) |
...
| [`client-work-with-structure`](skills/client/client-work-with-structure/SKILL.md) | pages / components / `src/lib` helpers / boot / stores placement (always-button brand logo ≥60px; Game leave via logo + status in App; auth→lobby; no page «В лобби»; support + content stores; content live Edit in-collection + trash/click isolation / staff unpublish/republish / author «На модерации» / three-phase marks / cascade outline + slot chips / draftStale pull / author delete unpublished / D1′ / staff reject-stays + approve-from-rejected + hub `tasksOnly` / no block UI; password policy helpers) |
| [`work-with-forms`](skills/client/work-with-forms/SKILL.md) | LoginPage / ForgotPasswordPage / ResetPasswordPage / AccountPage / Support create+reply `q-form` (policy meter on register/reset/change; `lazy-rules` + clear → `nextTick` → `resetValidation`) / guest + Google buttons |
| [`work-with-pages`](skills/client/work-with-pages/SKILL.md) | Routes + guards login/forgot/confirm/reset/account/lobby/support/content/admin/game; App header (always-button brand logo ≥60px + theme + verify reminder + Game leave via logo/status; auth→lobby; lobby noop; no page «В лобби»; productName/favicon); Lobby create grilleDensity+catapultDensity + join busy-lock + Support + «Наборы»→collection; content author «На модерации» + three-phase marks + cascade outline + slot chips + draftStale pull + staff unpublish/republish + task-set unpublish + subtitle=`taskSetStatusMarks.*` (published/approved) + live Edit in-collection + trash/click isolation + author delete + D1′ + staff reject-stays + approve-from-rejected + tasks-only hub / no block UI; staff filters; single ticket Close (`canClose`); admin `emailVerified` badge; GamePage top opponents + seated HUD strip / budgets beside / end-turn icon on avatar / return strip icon (no modal) / push icons / nearest center / catapult land→overlay→fling + deferred grille + continuous board-busy / presence row reserve / dual end + all-jail overview |
| [`work-with-stores`](skills/client/work-with-stores/SKILL.md) | Pinia `auth` (forgot/confirm/reset/change-email/displayName/changePassword + `role`/`isStaff`/`isAdmin`) / `theme` / `game` (incl. peeks∞ / finite steps / peek / rescue/push/return / grilleDensity+catapultDensity create / reveal keys / D13 atomic `$patch` seats+revealing / end-turn / holes; clear rooms until lobby snapshot) / `support` (tickets/staff/admin HTTP; `setUserRole` merge keeps `emailVerified`) / `content` (dual submit + my-moderation / three-phase / cascadeGap* / draftStale+rebase / unpublish/republish / D1′/D5′ on open=pending\|rejected / live `inCollection` + open author ids / trash confirm / author delete unpublished / staff pending\|rejected + approve-from-rejected / `not_approvable` / `tasksOnly` / no block UI; topic [content.md](skills/client/work-with-stores/content.md)) |
| [`work-with-styles`](skills/client/work-with-styles/SKILL.md) | Quasar Dark + GET/POST `/api/theme`, header always-button brand-logo ≥60px CSS + theme toggle, muted chrome, tourist board (holes + grille overlays / `--grille-anim-ms` 1000) + catapult reveal/broken-hold CSS + top presence + seated strip HUD / budgets beside / end-turn icon / peek-rescue-push top-center / say top↓ own↑ CSS |
| [`work-with-localization`](skills/client/work-with-localization/SKILL.md) | vue-i18n boot; auth-email RU + spam hint (forgot/confirm/reset SPA); password policy / strength / displayName / change-password keys; `auth.backToLobby` aria-only for always-button logo (lobby noop); `support.*` topics/statuses/filters/emailUnverified/errors/roles; `content.*` («Набор карточек» / `deleteCardConfirm*` by hasLive / `myModeration*` / `taskSetStatusMarks.*` three-phase + published/approved + page subtitle sync (`statusCycle*` aliases) / `unpublishedByStaff` / `draftStale*` / `unpublish*` / `republish*` / staff rejected label + nested + `content.errors.*`); `game.say` / ready / leave (logo aria) / finish / returnAffordance / pushAffordance / timer+steps end / steps/peeks / endTurn / peek / solo-peeks∞ / grille+catapult density + rescue/return/all-jail keys |
...
| [`work-with-test`](skills/client/work-with-test/SKILL.md) | Vitest + `@vue/test-utils` (harness live: `vitest.config.ts` + `test/setup.ts` + `npm test`; pages/components/stores `auth`/`theme`/`game`/`support`/`content` (my-moderation / three-phase / cascadeGap* / hasLive confirm / publish UX SC-PACK-85…96 / staff reject-stays + approve-from-rejected / `tasksOnly`)/lib; q-form; Pinia; Colyseus `client` spies; store `error` + `q-banner`; topic files: forms/stores/colyseus/errors/plugins/composables/provide-inject) |
...
| [`server-locate-change-points`](skills/server/server-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. `config/auth`, `lib/passwordPolicy`, `lib/support`, `lib/content` dual answers\|tasks + cascadeNormalize / D1′/D5′/author delete/my-moderation/reject-stays/approve-from-rejected/staff queue tasks-only + hub `tasksOnly`/`answersActionsAvailable` + unpublish/republish/rebase/`last_live_*`, support/content/admin + auth profile HTTP) |
...
| [`server-work-with-structure`](skills/server/server-work-with-structure/SKILL.md) | rooms / schema / db / config / lib (mailer + support + content dual answers\|tasks + cascadeNormalize ≠ false `answers_dirty` + live `inCollection`/pending ids + D1′/author delete + my-moderation + reject-stays + approve-from-rejected + staff queue tasks-only + hub `tasksOnly`/`answersActionsAvailable` + unpublish/republish/rebase + passwordPolicy) / app.config placement |
| [`server-work-with-test`](skills/server/server-work-with-test/SKILL.md) | mocha + `@colyseus/testing` (SC-PIECE + SC-START + SC-MOVE peeks∞/solo become-current +1 / step-loss/holes/grilles trap-rescue-push-return/catapult paced+deferred+idle re-eval SC-MOVE-78…93/leave-clear + SC-BOARD + SC-FINISH + SC-SAY + lobby density 12/22/35% + `GET|POST /api/theme` + auth email/policy + `zz-authProfile` SC-PROFILE + `test/support.test.ts` SC-SUP/SC-ROLE incl. create-ack/filters/no self-reply mail/role `emailVerified` + `zz-contentPacks.test.ts` SC-PACK dual answers|tasks + cascade SC-PACK-07/78–80 + live `inCollection` (SC-PACK-61…64) + D1′/author delete/reject-stays/approve-from-rejected/my-moderation + staff queue tasks-only + hub `tasksOnly` + publish UX SC-PACK-90…96) |
...
| [`work-with-routes`](skills/server/work-with-routes/SKILL.md) | HTTP `createEndpoint` / thin routes (incl. `GET|POST /api/theme`, auth email JSON + display-name/change-password, `/api/support/*` create-ack + staff query filters, `/api/content/*` dual submit + cascadeNormalize ≠ false `answers_dirty` + live open author ids + D1′/D5′ on open + author delete + `my-moderation` + reject-stays + approve-from-rejected (`not_approvable`) + staff queue tasks-only + hub `tasksOnly`/`answersActionsAvailable` + unpublish/republish/draft rebase/task-set unpublish, `/api/admin/*` non-anonymous + `emailVerified`) |
...
| [`work-with-database`](skills/server/work-with-database/SKILL.md) | GameDatabase, users schema (incl. nullable `theme`, `emailVerified`, `htRole`) + support_tickets/messages + content_* pack tables (`type` answers\|tasks, open pending\|rejected, snapshots/`tasksDirty`/`needsModeration` + three-phase + `last_live_*` + draft `based_on_live_revision_id` + cascade slot clear ≠ D1′ lock + D1′/author delete, `ensureContentTables`) |
