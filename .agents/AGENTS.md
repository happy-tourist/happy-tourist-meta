...
| [`client-locate-change-points`](skills/client/client-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. Google auth, support/content/admin pages + `stores/support` / `stores/content` dual submit + D1′/D5′/author delete / live Edit in-collection + trash/`inCollection` / hide Edit if `blocked` / staff redirect; Vitest harness) |
...
| [`server-locate-change-points`](skills/server/server-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. `config/auth`, `lib/passwordPolicy`, `lib/support`, `lib/content` dual answers\|tasks + D1′/D5′/author delete/staff nested, support/content/admin + auth profile HTTP) |
