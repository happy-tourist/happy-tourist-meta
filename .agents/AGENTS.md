...
| [`client-locate-change-points`](skills/client/client-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. Google auth, support/content/admin pages + `stores/support` / `stores/content`) |
...
| [`client-work-with-structure`](skills/client/client-work-with-structure/SKILL.md) | pages / components / `src/lib` helpers / boot / stores placement (always-button brand logo ≥60px; Game leave via logo + status in App; auth→lobby; no page «В лобби»; support + content stores; password policy helpers) |
...
| [`work-with-localization`](skills/client/work-with-localization/SKILL.md) | vue-i18n boot; auth-email RU + spam hint (forgot/confirm/reset SPA); password policy / strength / displayName / change-password keys; `auth.backToLobby` aria-only for always-button logo (lobby noop); `support.*` topics/statuses/filters/emailUnverified/errors/roles; `content.*` packs catalog/collection/editor/staff + errors; `game.say` / ready / leave (logo aria) / finish / returnAffordance / pushAffordance / timer+steps end / steps/peeks / endTurn / peek / solo-peeks∞ / grille+catapult density + rescue/return/all-jail keys |
| [`work-with-lobby`](skills/client/work-with-lobby/SKILL.md) | Live LobbyRoom subscribe, create-with-maxSeats + grilleDensity + catapultDensity modal (12/22/35% seed each; no Play shortcut), join busy-lock, clear stale rooms until snapshot, quiet resubscribe; header links Support + content packs («Наборы») |
...
| [`work-with-test`](skills/client/work-with-test/SKILL.md) | Vitest + `@vue/test-utils` (bootstrap on first test; pages/components/stores/lib; q-form; Pinia; Colyseus `client` spies; store `error` + `q-banner`; topic files: forms/stores/colyseus/errors/plugins/composables/provide-inject) |
...
| [`server-locate-change-points`](skills/server/server-locate-change-points/SKILL.md) | Где править / куда класть новые файлы (вкл. `config/auth`, `lib/passwordPolicy`, `lib/support`, `lib/content`, support/content/admin + auth profile HTTP) |
...
| [`work-with-routes`](skills/server/work-with-routes/SKILL.md) | HTTP `createEndpoint` / thin routes (incl. `GET|POST /api/theme`, auth email JSON + display-name/change-password, `/api/support/*` create-ack + staff query filters, `/api/content/*` packs catalog/collection/draft/moderation/staff, `/api/admin/*` non-anonymous + `emailVerified`) |
...
| [`work-with-database`](skills/server/work-with-database/SKILL.md) | GameDatabase, users schema (incl. nullable `theme`, `emailVerified`, `htRole`) + support_tickets/messages + content_* pack tables (`ensureContentTables`) |
