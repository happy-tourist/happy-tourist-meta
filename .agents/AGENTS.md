...
| [`work-with-game-board`](skills/client/work-with-game-board/SKILL.md) | Board + dual presence rings + place/timeout modals + ready/countdown + say + strip finish + leave confirm + `sendMove`/`sendReady`/`sendSay` |
...
| [`server-work-with-test`](skills/server/server-work-with-test/SKILL.md) | mocha + `@colyseus/testing` (SC-PIECE deferred pieces + SC-START + SC-MOVE timer + SC-FINISH + SC-SAY + lobby + `GET|POST /api/theme`) |
| [`work-with-rooms`](skills/server/work-with-rooms/SKILL.md) | Room lifecycle (`onDrop`/`onReconnect`/`onDispose` clearTurnDeadline), registration, maxSeats / phase / seats |
...
| [`work-with-game`](skills/server/work-with-game/SKILL.md) | Seating + deferred pieces + start/ready/countdown + reconnect + turn deadlines (60s/solo 5min) + center finish + one-step move (`touristMove.ts`); room `tourist` |
