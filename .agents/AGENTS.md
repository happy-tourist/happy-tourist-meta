...
| [`work-with-pages`](skills/client/work-with-pages/SKILL.md) | Routes + App shell (section nav / crumbs **below** elevated header / session logout / Game leave); Lobby room-list only; unified packs filters/star/`openRequestType`; collection redirect; author re-edit; staff take; MapsList/MapEditor view vs paint + title-row Edit |
| [`work-with-stores`](skills/client/work-with-stores/SKILL.md) | Pinia domains; `content` unified list + `openRequestType` + cancel→draft + set marks / `neverLive` ([content.md](skills/client/work-with-stores/content.md)); `maps` statuses/author→Edit/title-row Edit/cancel→draft ([maps.md](skills/client/work-with-stores/maps.md)) |
| [`work-with-styles`](skills/client/work-with-styles/SKILL.md) | Quasar Dark + GET/POST `/api/theme`, header always-button brand-logo ≥60px CSS + theme toggle, page-zone `.app-breadcrumbs` below elevated header, muted chrome, tourist board (holes + grille overlays / `--grille-anim-ms` 1000) + catapult reveal/broken-hold CSS + top presence + seated strip HUD / budgets beside / end-turn icon / peek-rescue-push top-center / say top↓ own↑ CSS; content `.cascade-gap-outline` (SC-PACK-126) |
| [`work-with-localization`](skills/client/work-with-localization/SKILL.md) | vue-i18n; `header.*` section nav; `content.*` / `maps.*` filters/statuses (published list rows: no badge); errors `author_request_open`/`moderation_taken`; support/auth/game |
| [`work-with-lobby`](skills/client/work-with-lobby/SKILL.md) | Live LobbyRoom; Packs/Maps/Support/Модерация/logout in App header — not LobbyPage |
...
| [`work-with-test`](skills/client/work-with-test/SKILL.md) | Vitest; ContentUnifiedList/`openRequestType` + AuthorEditTake/`neverLive` + maps SC-MAP-50…52 + AppHeaderChrome crumbs-below-header + soft-unpublish FollowUp/ModerationUx; topic files |
...
| [`server-locate-change-points`](skills/server/server-locate-change-points/SKILL.md) | Где править (content unified list/favorites/author re-edit/staff take/cancel→draft/set marks without fan-out/`neverLive`/`openRequestType`; maps statuses; default grants retired SC-PACK-170) |
...
| [`server-work-with-structure`](skills/server/server-work-with-structure/SKILL.md) | rooms/schema/db/config/lib (content unified list + favorites + author re-edit + staff take + working≠live draft + set marks without fan-out + `neverLive` + `openRequestType`; maps; default grants no-op; support/mailer) |
| [`server-work-with-test`](skills/server/server-work-with-test/SKILL.md) | mocha; zz-contentPacks unify/favorites/author-edit/take/cancel→draft/set marks + fan-out fix/`neverLive` SC-PACK-187…189; zz-contentMaps; zz-defaultContentPacks SC-PACK-170; rooms/auth/support |
...
| [`work-with-routes`](skills/server/work-with-routes/SKILL.md) | HTTP thin routes; `/api/content/*` unified list (`openRequestType`; working≠live → draft) + favorite + author draft/lock + set `moderationStatus` / `neverLive` ghost + staff take/release + maps; collection legacy; grants retired |
