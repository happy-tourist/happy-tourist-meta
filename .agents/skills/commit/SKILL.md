...
name: commit
description: >-
  Scans uncommitted changes across happy-tourist-meta, happy-tourist.github.io,
  and happy-tourist-server; always stages, commits on main/master, and pushes.
  Use when the user runs the commit skill or asks to commit across the ecosystem
  («закоммитить»); push is always included — no separate push request needed.
---

# Commit — три репозитория экосистемы

Проверить незакоммиченные изменения во всех трёх проектах, застейджить, закоммитить и **всегда запушить**. Запуск этого скилла = полный цикл `stage → commit → push`. Отдельное слово «пуш» в запросе не требуется.

Рабочие ветки: **main** или **master**.

## Когда применять

Только если пользователь **явно** запускает commit / просит закоммитить по экосистеме (этот скилл), **или** оркестратор `implement-change` / `end-implement-change` делегирует эту фазу. Не вызывать «на всякий случай» после правок кода.
...
|------|-------------|--------------|----------------|
| meta (этот репо) | `happy-tourist-meta` | `.` | `main` |
| `client` | `happy-tourist.github.io` | `../happy-tourist.github.io` | `master` |
| `server` | `happy-tourist-server` | `../happy-tourist-server` | `main` |
...
### 2. Status scan (параллельно по трём репо)
...
Показать пользователю кратко **до** stage/commit (что будет закоммичено в каком репо). Если все три чистые — остановиться: «нечего коммитить».
...
- Если пользователь дал одно общее сообщение — использовать его во всех репо с изменениями (допустимо лёгкий префикс `client:` / `server:` / `meta:` только если это улучшает ясность).
...
- «Проверь незакоммиченные изменения в meta/client/server и закоммить»
- «Сделай commit по экосистеме»
- Фаза commit из `implement-change` / `end-implement-change`
...
- Не трогать только один репо, если пользователь просил «все три» — просканировать все; коммитить только dirty.
