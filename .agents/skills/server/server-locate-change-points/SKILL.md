...
| Lib | `src/lib/mailer.ts`, `src/lib/support.ts`, `src/lib/content.ts` | smtp.bz mail; support tickets/roles/bootstrap/auto-close; content packs (ensure + dual answers\|tasks + live `inCollection`/open author ids + **D1′**/D5′ on open/author delete/snapshot rewrite/`cascadeNormalizeTasks`/`listMyModerationPacks`/`listPendingPacks` reject-stays + tasks-only + preview `tasksOnly`/`answersActionsAvailable`/approve-from-rejected/`not_approvable`/staff hub nested; reject mail «Нужна доработка») |
...
| Content packs (live/draft/collection; dual answers\|tasks; live `inCollection` + open author ids (`pending*AuthorId` = pending\|rejected); statuses/`tasksDirty`/`needsModeration` + three-phase + `draftStale`; **D1′**/D5′ on open; cascadeNormalize ≠ false `answers_dirty`; author delete unpublished (forbid while `last_live_*`); staff `unpublishPack`/`republishPack`/`unpublishLiveTaskSet`/`rebaseDraft`; snapshot rewrite after approve; staff reject-stays + approve-from-rejected + `my-moderation` + tasks-only hub `tasksOnly`/`answersActionsAvailable`) | `src/db/schema.ts` `content_*` (moderation `type`, snapshots, `live_tasks_revision_id`, `last_live_*`, draft `based_on_live_revision_id`) + `src/lib/content.ts` (`ensureContentTables`, `getLivePack`, `getOpenModerationRequest`, `submitAnswers`/`submitTasks`, `unpublishPack`/`republishPack`/`unpublishLiveTaskSet`/`rebaseDraft`, `listMyModerationPacks`, `listPendingPacks` pending\|rejected, `approveRequest` from rejected → else `not_approvable`, reject mail «Нужна доработка», `deleteUnpublishedPack`/`deleteTaskSet`, D1′/D5′; D20 mail → `#/…/edit` or `#/…/tasks/:setId`); thin `POST …/unpublish` `/republish` `/draft/rebase` `/task-sets/:id/unpublish` + `POST /api/content/pack/delete` + `/task-set/delete` + `GET /api/content/my-moderation` in `app.config.ts`; create/edit/submit require non-anonymous + `emailVerified` from DB |
...
| Content packs (SC-PACK-*) | `test/zz-contentPacks.test.ts` — mock mailer; `ensureContentTables` / `setEmailVerifiedForTests`; dual submit answers\|tasks; live `inCollection` + pending author ids (SC-PACK-61…64); **D1′** + D5′ locks; cascade save SC-PACK-07/78–80; author delete unpublished pack/task-set; snapshot marks after approve; draft statuses/`tasksDirty`/`needsModeration` + three-phase; staff reject-stays + approve-from-rejected + `my-moderation` (SC-PACK-19/73…76) + tasks-only hub `tasksOnly`/`answersActionsAvailable` (SC-PACK-30/70…72) → nested + approve order; publish UX SC-PACK-90…96 (`draftStale`/rebase, staff unpublish/republish, task-set unpublish) |
