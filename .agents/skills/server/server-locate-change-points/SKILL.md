...
| Lib | `src/lib/mailer.ts`, `src/lib/support.ts`, `src/lib/content.ts` | smtp.bz mail; support tickets/roles/bootstrap/auto-close; content packs (ensure + dual answers\|tasks + live `inCollection`/open author ids + **D1′**/D5′ on open/author delete/snapshot rewrite/`listMyModerationPacks`/`listPendingPacks` reject-stays + tasks-only + preview `tasksOnly`/`answersActionsAvailable`/approve-from-rejected/`not_approvable`/staff hub nested; reject mail «Нужна доработка») |
...
| Content packs (SC-PACK-*) | `test/zz-contentPacks.test.ts` — mock mailer; `ensureContentTables` / `setEmailVerifiedForTests`; dual submit answers\|tasks; live `inCollection` + pending author ids (SC-PACK-61…64); **D1′** + D5′ locks; author delete unpublished pack/task-set; snapshot marks after approve; draft statuses/`tasksDirty`/`needsModeration` + three-phase; staff reject-stays + approve-from-rejected + `my-moderation` (SC-PACK-19/73…76) + tasks-only hub `tasksOnly`/`answersActionsAvailable` (SC-PACK-30/70…72) → nested + approve order |
