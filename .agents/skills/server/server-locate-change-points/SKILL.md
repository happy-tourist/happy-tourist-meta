...
| OAuth config | `src/config/auth.ts` | `auth.oauth.addProvider('google', …)`; `htRole` → userdata `role`; wrap `onOAuthProviderCallback` for verified + default-pack grant on new create; wrap email/anonymous register for default packs |
| Lib | `src/lib/mailer.ts`, `src/lib/support.ts`, `src/lib/content.ts`, `src/lib/contentMaps.ts`, `src/lib/defaultContentPacks.ts` | smtp.bz mail; support tickets/roles/bootstrap/auto-close + `change_pack`/`pack_id`; content packs (ensure + working copy + unified `submitPack`/approve + add-task-set + staff edit lock/save + soft-unpublish pack+set/`in_catalog` + pack unpublish cascade-cancel open req + mail SC-PACK-137…141 + previewPending live cards + authorDisplayName + needs-revision + `cascadeNormalizeTasks` + author delete + `listMyModerationPacks` / staff pending queue — maps branch delegates to `contentMaps`); content maps (ensure + draft/submit/approve/needs_revision/soft-unpublish cascade-cancel + staff lock/save + author delete; SC-MAP); default packs (`DEFAULT_CONTENT_PACK_IDS` parse/grant/backfill SC-PACK-142…147) |
...
| Tests | `test/` | mocha + `@colyseus/testing` (incl. `support.test.ts`, `zz-contentPacks.test.ts`, `zz-contentMaps.test.ts`, `zz-defaultContentPacks.test.ts`) |
...
| Content packs (working copy until first approve; unified submit; add-task-set; staff lock/save; soft-unpublish pack+set/`in_catalog` + unpublish/republish; needs_revision; cascadeNormalize ≠ false `answers_dirty`; author delete unpublished; my-moderation + staff pending queue; **default packs** `DEFAULT_CONTENT_PACK_IDS` one-shot grant) | `src/db/schema.ts` `content_*` (`working_revision_id`, `edit_locked_*`, pack + task-set `in_catalog`, moderation `type` pack\|task_set; migrate drop `content_user_drafts`) + `src/lib/content.ts` (`ensureContentTables`, `submitPack`/`submitAddTaskSet`, `acquireEditLock`/`staffSavePack`, `unpublishPack`/`republishPack`, `unpublishTaskSet`/`republishTaskSet`, `needsRevision`/`approveRequest`, `deleteUnpublishedPack`/`deleteTaskSet`, `cascadeNormalizeTasks`, `listMyModerationPacks`/`listPendingPacks`) + `src/lib/defaultContentPacks.ts` (parse/eligible/grant/backfill); create-user hooks in `src/config/auth.ts`; boot backfill in `app.config.ts` `express()`; thin routes in `app.config.ts` (incl. `POST /api/content/task-set/unpublish\|republish`); create/edit/submit require non-anonymous + `emailVerified` from DB |
...
| Content packs HTTP | `src/lib/content.ts` + `src/lib/defaultContentPacks.ts` + thin `/api/content/*` in `src/app.config.ts` (pack + `task-set` unpublish/republish); tests `test/zz-contentPacks.test.ts` + `test/zz-defaultContentPacks.test.ts` |
