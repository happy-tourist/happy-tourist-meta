...
| Content packs (unified list + favorites + author re-edit + staff take; soft-unpublish; add-task-set no collection; cancel keeps working → list draft; set `moderationStatus`; never-live cancel → `getAddTaskSet` restore + put/submit reuse cancelled revision D22; DEFAULT grants retired SC-PACK-170) | `src/db/schema.ts` (`content_pack_favorites`, moderation `taken_by`/`taken_at`) + `src/lib/content.ts` (`retainedNeverLiveAuthorTaskSetCycle`, `getAddTaskSet`/`putAddTaskSet`/`submitAddTaskSet`) + `defaultContentPacks.ts` (no-op grant) + thin routes in `app.config.ts` |
...
| Content packs (SC-PACK-*) | `test/zz-contentPacks.test.ts` — mock mailer; `ensureContentTables` / `setEmailVerifiedForTests`; working copy + unified submit SC-PACK-100…105; add-task-set SC-PACK-108…110; staff lock/save SC-PACK-111…113; author delete SC-PACK-114; soft-unpublish pack SC-PACK-120…124; cascade-cancel open requests + mail SC-PACK-137…141; task-set soft-hide SC-PACK-131 (`last_published_task_set`); SC-PACK-134…135 preview/authorDisplayName; cascade save; my-moderation + staff pending\|needs_revision; cancel→draft / set marks / fingerprint SC-PACK-171…179; fan-out fix + neverLive ghost SC-PACK-187…189; cancel never-live GET restore + amend/resubmit SC-PACK-196/197 |
