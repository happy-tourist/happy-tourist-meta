...
| Lib | `src/lib/mailer.ts`, `src/lib/support.ts`, `src/lib/content.ts`, `src/lib/defaultContentPacks.ts` | smtp.bz mail; support tickets/roles/bootstrap/auto-close + `change_pack`/`pack_id`; content packs (ensure + working copy + unified `submitPack`/approve + add-task-set + staff edit lock/save + soft-unpublish pack+set/`in_catalog` + pack unpublish cascade-cancel open req + mail SC-PACK-137…141 + previewPending live cards + authorDisplayName + needs-revision + `cascadeNormalizeTasks` + author delete + `listMyModerationPacks` / staff pending queue); default packs (`DEFAULT_CONTENT_PACK_IDS` parse/grant/backfill SC-PACK-142…147) |
...
| Tests | `test/` | mocha + `@colyseus/testing` (incl. `support.test.ts`, `zz-contentPacks.test.ts`, `zz-defaultContentPacks.test.ts`) |
