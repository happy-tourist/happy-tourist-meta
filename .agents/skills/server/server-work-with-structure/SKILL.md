...
| Mailer / support / content / password policy | `src/lib/` | `mailer.ts`; `support.ts`; `content.ts` — unified list + favorites + author re-edit + staff take + working≠live → list `draft` + per-set `moderationStatus` (SC-PACK-171…179) + soft-unpublish + my-moderation/staff queue; `contentMaps.ts` — map list statuses + cancel→draft when working differs (SC-MAP-41/42) + soft-unpublish; `defaultContentPacks.ts` — parse/eligible; grant/backfill **no-ops** (SC-PACK-170); `passwordPolicy.ts` |
