...
| Mailer / support / content / password policy | `src/lib/` | `mailer.ts` — smtp.bz `sendEmail` (+ test setter); `support.ts` — tickets/messages/roles/bootstrap/auto-close + create-ack mail + staff list filters + `setTicketStatus(..., { notify })` (HTTP stays thin); `content.ts` — packs catalog/collection/draft (statuses/`tasksDirty`/`needsModeration` + three-phase + `draftStale`); live GET `inCollection` + open author ids; dual `submitAnswers`/`submitTasks`; **D1′**/D5′ on **open** = pending\|rejected; `cascadeNormalizeTasks` / slot clear ≠ `answers_dirty`; author `deleteUnpublishedPack`/`deleteTaskSet` (forbid while `last_live_*`); staff `unpublishPack`/`republishPack`/`unpublishLiveTaskSet`/`rebaseDraft`; snapshot rewrite after approve; type-scoped threads; `listMyModerationPacks`; `listPendingPacks` answers + tasks-only + open rejected; `approveRequest` from pending\|rejected (`not_approvable` else); reject mail «Нужна доработка»; preview `tasksOnly`/`answersActionsAvailable`; staff hub → nested tasks; moderation mail deep-links D20 → `#/…/edit` or `#/…/tasks/:setId` (not bare `/moderation`); `passwordPolicy.ts` — shared ≥8 + lower/upper/digit/symbol (register wrap / JSON reset / change-password) |
...
├── content.ts         # content packs (ensureContentTables + dual answers|tasks + live inCollection/pending ids + cascadeNormalize ≠ answers_dirty + D1′/author delete/listMyModeration + listPending reject-stays + approve-from-rejected + preview tasksOnly/answersActionsAvailable)
