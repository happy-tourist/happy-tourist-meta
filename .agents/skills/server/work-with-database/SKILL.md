...
`support_tickets` / `support_messages` are declared in `src/db/schema.ts` and created at boot by `ensureSupportTables()` in `src/lib/support.ts` (raw `CREATE TABLE IF NOT EXISTS` + `ALTER` for `pack_id` on older DBs) — they are **not** registered in `GameDatabase` `schemas: { users }`. Topic `change_pack` stores catalog `pack_id` (SC-SUP-27). Logic lives in `support.ts`; HTTP stays thin in `app.config.ts`.

### Content pack tables (custom, not SchemaSet)

`content_packs`, `content_pack_revisions`, `content_answer_cards`, `content_task_sets`, `content_tasks`, `content_task_slots`, `content_pack_collections`, `content_moderation_requests`, `content_moderation_messages` — declared in `src/db/schema.ts`, created at boot by `ensureContentTables()` in `src/lib/content.ts` (migrate/drop `content_user_drafts`; `ALTER` for `working_revision_id` / `edit_locked_by` / `edit_locked_at` / `in_catalog`). Single creator **working copy** until first catalog approve (`working_revision_id`). Soft-hide: `in_catalog` boolean (default false; set true on first Approve / republish; false on staff unpublish; live revision retained). Moderation request `type` `pack`|`task_set` (legacy answers|tasks migrated); **open** = `pending`|`needs_revision` (no hard-reject). Staff exclusive edit lock columns on `content_packs`. Cascade slot clears from card content/delete (`cascadeNormalizeTasks`) MUST NOT count as tasksChanging under `answers_dirty`. Author delete unpublished pack/task-set. Live vs working: public GETs return approved **in-catalog** live snapshot; staff may GET soft-unpublished; unpublished working copy via `/draft` for creator only. Not SchemaSet / not room state.
