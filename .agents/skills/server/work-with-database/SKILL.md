...
`content_maps`, `content_map_revisions` — declared in `src/db/schema.ts`, ensured via `ensureContentMapTables()` (called from `ensureContentTables`). Map grid is 100-char alphabet `.`|`1`|`*`|`7`; soft-hide via `content_maps.in_catalog`; shared moderation rows with `type=map` + `map_id`. Lib: `src/lib/contentMaps.ts`.
