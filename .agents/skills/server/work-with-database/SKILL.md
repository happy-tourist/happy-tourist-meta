...
`content_packs`, `content_pack_revisions`, `content_user_drafts`, `content_answer_cards`, `content_task_sets`, `content_tasks`, `content_task_slots`, `content_pack_collections`, `content_moderation_requests`, `content_moderation_messages` — declared in `src/db/schema.ts`, created at boot by `ensureContentTables()` in `src/lib/content.ts` (same ensure-at-boot pattern as support; `ALTER` migrate for new draft/pack cols). Dual moderation: request `type` `answers`|`tasks` (one pending per `(pack, type)`); **open** = `pending`|`rejected` (reject stays in queue / «нужна доработка»; approve-from-rejected OK; cancel drops). Drafts keep `last_answers_snapshot` + `last_tasks_snapshot` for dirty/`tasksDirty` / per-set marks; per-task-set `needsModeration` (rewrite snapshots after approve/`copyRevision`); draft GET exposes answers/tasks statuses + open-thread requestId (three-phase: ожидает отправки / на модерации / нужна доработка) + `draftStale` when `based_on_live_revision_id` ≠ pack `live_revision_id`; packs may have separate `live_tasks_revision_id` and staff-unpublish retainers `last_live_revision_id` / `last_live_tasks_revision_id` (republish restores catalog without new moderation). Draft rows track `based_on_live_revision_id` for rebase overlays. **D1′:** dirty with open pending|rejected → lock tasks for all; answers-pending author A MAY create/edit tasks while dirty; others blocked. Cascade slot clears from card content/delete (`cascadeNormalizeTasks`) MUST NOT count as tasksChanging under `answers_dirty`; description-only / unused delete = no cascade. D5′: only answers-pending author may resubmit answers; others cannot submit tasks while answers pending. Author delete unpublished pack (cascade) / task set (sync draft + liveTasks); forbid hard-delete while `last_live_*` retained without live. Live vs draft: public GETs return approved live snapshot; pending drafts stay separate. Not SchemaSet / not room state.
