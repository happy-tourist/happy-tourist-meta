...
description: >-
  ...
  holdingGrilleKeys + revealingCatapultKeys + brokenCatapultKeys, MapSchema /
  ArraySchema, schema() + t.* (v5), or aligning the sync surface with the
  sibling client tourist contract.
---
...
`turnBudgetSeconds`/`removedTaskKeys`/`holdingGrilleKeys`/`revealingCatapultKeys`/
`brokenCatapultKeys`/connectivity/ready/`finishPlace`/`timeExpired`/piece `trapped`
mirrored in `stores/game`; presence dual rings + own budgets + move/peek/rescue/grille/
catapult UX on GamePage).
...
task reward bag, hidden grille + hidden catapult seed, turn deadlines, and
...
export const MyRoomState = schema(
  {
    ...
    /**
     * Task cells removed after **correct** peek only (`"r,c"`). Not landable; hole chrome on client.
    ...
    /**
     * Catapult cells currently presenting fade reveal (`"r,c"`).
     * Hidden unspent catapults are room-private; clear after consume anim.
     */
    revealingCatapultKeys: t.array("string"),
    /**
     * Subset of revealing (or just-consumed) catapults that show broken artwork.
     */
...
**Not synced (room-private + messages):** step/peek budgets (`budgets` Map + `client.send('budgets')`); task reward bag (`taskRewards`); open peek (`openPeek` + `client.send('peekOpen')`); create-time `grilleDensity` / `catapultDensity` + hidden grille/catapult keys; private `allJailWarning`. Do **not** put steps/peeks or hidden trap locations on schema.
...
- Client mirrors `phase` / `maxSeats` / `countdownRemaining` + seats (+ `ready` / `finishPlace` / `timeExpired` / piece `finished`/`trapped`) + `currentTurnSessionId` / `turnUntil` / `turnBudgetSeconds` / `removedTaskKeys` / `holdingGrilleKeys` / `revealingCatapultKeys` / `brokenCatapultKeys` into Pinia; GamePage draws unfinished pieces (+ short disappear), grille overlays, catapult fade overlays, holes for removed `*`, strip only once pieces exist, place/timeout/peek/solo/all-jail modals, leave confirm only when seated ∧ playing ∧ `finishPlace === 0` ∧ `!timeExpired`, and say bubbles from `sayEvents`. Own steps/peeks come from private `budgets`, not schema.
...
- Keep schema fields to **what must sync** (today: `phase` / `maxSeats` / `countdownRemaining` + `seats` incl. connectivity/`ready`/`finishPlace` + piece `finished`/`trapped` + `currentTurnSessionId` + `nextFinishPlace` + `removedTaskKeys` + `holdingGrilleKeys` + `revealingCatapultKeys` + `brokenCatapultKeys`; legacy `started` OK).
...
- Put rules, seating pools, `turnOrder`, countdown timers, reconnect timers, say/`liveSays`, private step/peek budgets, task reward bag, hidden grille/catapult seed/`grilleDensity`/`catapultDensity`, win detection, or rating DB writes inside schema files.
...
1. Field names match client Pinia / GamePage (`phase`, `maxSeats`, `countdownRemaining`, `seats` → `touristId` + `pieces` (+ `finished`/`trapped`) + connectivity + `ready` + `finishPlace`, `currentTurnSessionId`, `removedTaskKeys`, `holdingGrilleKeys`, `revealingCatapultKeys`, `brokenCatapultKeys`; client need not mirror `nextFinishPlace`).
2. Room owns seating, reconnect, start countdown, turn order, budgets/peek/grilles/catapults, and messages — schema does not define messages or `turnOrder` / steps/peeks / hidden traps.
