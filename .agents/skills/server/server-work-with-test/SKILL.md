...
description: >-
  ...
  dirty + D5′ lock; draft statuses/tasksDirty/needsModeration; staff nested hub; mock mailer; ensureContentTables / setEmailVerifiedForTests).
...
| Content packs (SC-PACK-*) | `test/zz-contentPacks.test.ts` — mock mailer; `ensureContentTables` / `setEmailVerifiedForTests` / `setUserRoleForTests`; cover catalog/collection, create+verify gate, dual submit answers|tasks minima, dirty-answers + D5′ pending-author lock, draft statuses/`tasksDirty`/`needsModeration`, type-scoped pending race/resubmit, moderation thread, staff answers-pending hub → nested tasks approve order + block + mail links; `keepLatestRequestListener` after boot |
