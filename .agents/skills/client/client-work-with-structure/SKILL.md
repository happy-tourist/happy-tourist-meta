...
description: >-
  ...
  auth→lobby; theme; no page «В лобби»; content packs working copy + add-task-set beside «Задания» + staff Edit lock session + soft-unpublish/`inCatalog` + trash/click isolation (no row `:to`; soft-unpublished gray; hide Edit if `blocked`) / author «На модерации» (hidden for staff) / cascade yellow + Editor/Tasks `cascade-gap-outline` CSS (SC-PACK-126) / slot chips on question lists (SC-PACK-127) / add-task-set status+thread (SC-PACK-128) / author delete unpublished / staff needs_revision / no block UI). No blocks/
...
| Content packs | `ContentCatalogPage` (staff unpublish/republish) / `ContentCollectionPage` (Edit unpublished creator/staff; soft-unpublished gray; **no** row add-task-set; trash; no row `:to`) / `ContentMyModerationPage` / `ContentPackPage` (staff Edit+lock + unpublish/republish; add-task-set beside «Задания» in-catalog; live slot chips) / `ContentPackCreatePage` / `ContentPackEditorPage` (`cascade-gap-outline` CSS on task-set rows) / `ContentPackAddTaskSetPage` (slot chips + open-request status/thread/reply) / `ContentPackTasksPage` (task yellow + slots) / `ContentPackModerationPage` / `ContentStaffPage` / `ContentStaffRequestPage` (Approve / needs-revision; **slot chips**; no block UI) / `ContentStaffTasksPage` (→ hub) | `stores/content` HTTP (`client.http`); working copy + `submitPack` + add-task-set + staff lock session + soft-unpublish (`unpublishPack`/`republishPack`, `inCatalog`) + cascadeGap* + SC-PACK-126…128 + needs_revision |
