...
**Game** — `GamePage` shows tourist board with all seats’ pieces (4 per seated) + occupied presence (offline grace ring) + strip×4 if seated; `rejoinGame(roomId)` on mount / soft-fail (reconnect token → `joinById`); no move UX until rules land.
