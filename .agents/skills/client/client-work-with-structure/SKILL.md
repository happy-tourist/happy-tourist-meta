...
description: >-
  ...
  auth→lobby; theme; no page «В лобби»; content packs live Edit in-collection + trash/click isolation (no row `:to`; hide Edit if `blocked`) / author «На модерации» / three-phase marks / author delete unpublished / D1′ / staff reject-stays + approve-from-rejected + hub `tasksOnly` / no block UI). No blocks/
...
| Content packs | `ContentCatalogPage` / `ContentCollectionPage` (Edit + trash + confirm; no row `:to`; hide Edit if `blocked`; nav → «На модерации») / `ContentMyModerationPage` (author «На модерации») / `ContentPackPage` (live Edit when `inCollection` + open-author; hide if `blocked`/foreign open) / `ContentPackCreatePage` / `ContentPackEditorPage` («Набор карточек» + three-phase + author delete) / `ContentPackTasksPage` (D1′/D5′ on open + three-phase + delete set) / `ContentPackModerationPage` / `ContentStaffPage` (answers + tasks-only + rejected queue; approve-from-rejected) / `ContentStaffRequestPage` (same hub; hide answers actions when tasks-only; no block UI) / `ContentStaffTasksPage` (→ hub, or queue if tasks-only) | `stores/content` HTTP (`client.http`); dual submit + my-moderation + D1′/D5′ on open + three-phase; live `inCollection` + open author ids; staff pending\|rejected/`tasksOnly`; lobby «Наборы» → `content-collection` |
