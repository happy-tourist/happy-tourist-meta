...
| Content packs | `ContentCatalogPage` / `ContentCollectionPage` (Edit + trash + confirm; no row `:to`; hide Edit if `blocked`; nav → «На модерации») / `ContentMyModerationPage` (author «На модерации») / `ContentPackPage` (live Edit when `inCollection` + open-author; hide if `blocked`/foreign open) / `ContentPackCreatePage` / `ContentPackEditorPage` («Набор карточек» + three-phase + hasLive delete confirm + cascade save + author delete) / `ContentPackTasksPage` (D1′/D5′ on open + yellow cascade gaps + task-row slots + subtitle=`taskSetStatusMarks.*` + delete set) / `ContentPackModerationPage` / `ContentStaffPage` (answers + tasks-only + rejected queue; approve-from-rejected) / `ContentStaffRequestPage` (same hub; hide answers actions when tasks-only; no block UI) / `ContentStaffTasksPage` (→ hub, or queue if tasks-only) | `stores/content` HTTP (`client.http`); dual submit + my-moderation + cascadeGap* + D1′/D5′ on open + three-phase; live `inCollection` + open author ids; staff pending\|rejected/`tasksOnly`; lobby «Наборы» → `content-collection` |
