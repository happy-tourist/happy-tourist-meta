...
| Game messages | `sendMove` → `move` (pieceId; no turn advance); `sendRescue` / `sendPush` / `sendReturnFromFinish`; `sendPeek` / `sendPeekPlace` / `sendPeekSubmit` / `sendEndTurn`; `sendReady` → `ready`; `sendSay` → `say`; listen `budgets` / `allJailWarning` / `say` (+ synced peek session) |
...
| Keep room I/O in Pinia `game` store | Call `room.send` from GamePage — use `sendMove` / `sendRescue` / `sendPush` / `sendReturnFromFinish` / `sendPeek` / `sendPeekPlace` / `sendPeekSubmit` / `sendEndTurn` / `sendSay` |
...
| Live state | `stores/game.ts` → `_attachRoom` | `onStateChange` (`seats`/`phase`/`maxSeats`/`grid`/`touristsPerPlayer`/peek session/`countdownRemaining`/`currentTurnSessionId`/`removedTaskKeys`/`holdingGrilleKeys`/`revealingCatapultKeys`/`brokenCatapultKeys`); `onMessage('say'|'budgets'|'allJailWarning')` (+ synced peek); `onError`, `onLeave` |
...
| `game` | `subscribeLobby`, `unsubscribeLobby`, `createGame`, `joinGame`, `rejoinGame`, `leaveGame`, `sendMove`, `sendRescue`, `sendPush`, `sendReturnFromFinish`, `sendPeek`, `sendPeekPlace`, `sendPeekSubmit`, `sendEndTurn`, `sendReady`, `sendSay` (`refreshRooms` HTTP unused) |
...
| `GamePage` | `game.rejoinGame(roomId)` on remount / soft-fail; pieces + grilles + presence + budgets + peek from store; move → `sendMove`; rescue/push/return → `sendRescue` / `sendPush` / `sendReturnFromFinish`; peek → `sendPeek` / `sendPeekPlace` / `sendPeekSubmit`; end-turn → `sendEndTurn`; say → `sendSay`; `leaveGame` |
...
| `revealingCatapultKeys` | Short-lived synced catapult cells for sequential client presentation (`"r,c"`); hidden unspent never sync; client `$patch` seats+revealing atomically (D13) |
...
| synced peek session / optional `peekOpen` | `openPeek` |
...
`GamePage` may call `rejoinGame(roomId)` if Pinia lost the room after refresh / soft-fail / browser reopen (`localStorage` reconnection token → `reconnect`, clear stale on fail → `joinById`); failed rejoin → navigate to lobby. Board tile geometry from synced `grid` (`lib/boardGeometry`); seats + phase + connectivity + turn + removed tiles + holding grilles + catapult reveal keys + peek session come from sync; own budgets from private messages. Token details: `work-with-rooms`.

## Messages: game actions

| Direction | Name | Payload |
|-----------|------|---------|
| Client → server | `move` | `{ pieceId, row, col }` via `sendMove` when playing + `isMyTurn` (spends step; **no** turn advance; may trap) |
| Client → server | `rescue` | `{ pieceId }` via `sendRescue` |
| Client → server | `push` | `{ pusherPieceId, targetSessionId, targetPieceId, row, col }` via `sendPush` (−1 step; relocates target only; no turn advance) |
| Client → server | `returnFromFinish` | `{ pieceId, row, col }` via `sendReturnFromFinish` |
| Client → server | `peek` / `peekPlace` / `peekSubmit` | `{ pieceId }` / place / submit — shared Q&A (no stub Correct/Wrong) |
...
| Server → owner | `peekOpen` | legacy optional; prefer synced peek session |
...
sendPush(
  ...
  if (!this.room || this.phase !== 'playing' || !this.isMyTurn) return false;
  this.room.send('push', { pusherPieceId, targetSessionId, targetPieceId, row, col });
  return true;
}
...
sendPeekPlace(slotIndex: number, answerCardId: string | null): boolean {
  if (!this.room || !this.openPeek || !this.isPeekOwner) return false;
  this.room.send('peekPlace', { slotIndex, answerCardId });
  return true;
}
...
```ts
// GamePage: synced grid + pieces + presence + budgets + shared peek + say; remount → rejoinGame(roomId)
// Own turn: select → red targets → game.sendMove(pieceId, row, col); eye → sendPeek / place / submit
...
| Calling `room.send(…)` from GamePage | Use `game.sendMove` / `sendRescue` / `sendPush` / `sendReturnFromFinish` / `sendPeek` / `sendPeekPlace` / `sendPeekSubmit` / `sendEndTurn` / `sendSay` only |
...
3. Lobby list: LobbyRoom subscribe; rooms: `create({ mapId, packId, taskSetIds, grilleDensity, catapultDensity })` / `joinById` (no Play shortcut); Game `sendMove` → `move` `{ pieceId, row, col }`; `sendRescue` / `sendPush` / `sendReturnFromFinish`; `sendPeek` / `sendPeekPlace` / `sendPeekSubmit` / `sendEndTurn`; `sendReady` → `ready`; `sendSay` → `say` + `onMessage('say'|'budgets'|'allJailWarning')`. HTTP only as unused fallback.
