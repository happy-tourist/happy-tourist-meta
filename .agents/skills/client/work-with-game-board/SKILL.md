...
| Catapults | Assets `src/assets/catapults/catapult.png` + `catapult-broken.png`; hidden until `revealingCatapultKeys`; **sequential queue** (no chain cap): pin piece on catapult cell during overlay → after full vanish → CSS travel `MOVE_ANIM_MS` to sync dest (or finish travel if center); successful fade ~**`CATAPULT_ANIM_MS = 1000`**; broken: intact → **300 ms** → broken → **300 ms** → vanish (no travel); fling→catapult chains play overlay→travel→next overlay (SC-BOARD-22…26, SC-FINISH-19) |
| Board lock | `isInteractive` false while `isBoardBusy`: own move travel, grille drop/rise, catapult overlay/holds/queue/pin/fling travel, finish disappear, return travel (SC-BOARD-27) |
...
- **Catapult fling (SC-BOARD-23…26 / SC-FINISH-19):** while overlay runs, pin piece on catapult cell (local override despite sync dest); **only after** full vanish start travel to dest / finish travel+fade; chain = sequential overlay→travel→next; broken holds 300+300 then vanish with no travel.
