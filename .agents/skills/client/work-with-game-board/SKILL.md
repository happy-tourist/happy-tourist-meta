...
description: >-
  ...
  + catapult sequential overlays (`CATAPULT_ANIM_MS=1000`, broken 300+300 hold,
    ...
    pin→vanish→travel, board-busy lock) + trap/rescue/push +
...
| Catapults | Assets `src/assets/catapults/catapult.png` + `catapult-broken.png`; hidden until `revealingCatapultKeys`; **every viewer** (player + spectator): **land/arrival** onto catapult cell (synthesize if sync already relocated; no fake step if already on cell) → overlay → fling/finish travel; **sequential queue** (no chain cap); **yield/cancel own `moveAnimating`** when queue starts; pin on catapult cell during overlay → after full vanish → CSS travel `MOVE_ANIM_MS` to sync dest (or finish travel if center); successful fade ~**`CATAPULT_ANIM_MS = 1000`**; broken: intact → **300 ms** → broken → **300 ms** → vanish (no travel); **D13:** store `$patch` seats+revealing atomically; watch `flush: 'pre'`; attribute by coords transition / last-known / sole mover; **no silent skip** (enqueue overlay even without piece key); **defer grille drop** while catapult queue busy (`pendingGrilleDropKeys` — SC-BOARD-29/30) (SC-BOARD-22…30, SC-FINISH-19) |
| Board lock | `isInteractive` false while `isBoardBusy`: own move travel, land-before-overlay, grille drop/rise, deferred grille pending, catapult overlay/holds/queue/pin/fling travel, finish disappear, return travel (SC-BOARD-27/29/30) |
...
- On submit: set `moveAnimating` (feeds `isBoardBusy`) and ignore board/strip clicks until timer ends; keep `selectedSide` unless the destination is center finish (SC-MOVE-46).
- Skip transition on first paint so pieces do not fly from origin.
- **Catapult fling (SC-BOARD-23…28 / SC-FINISH-19 / D13):** every viewer: **land onto catapult cell first** (synthesize when sync already at fling dest; skip fake step if already on cell / after in-flight arrival) → overlay → travel. While overlay runs, pin piece on catapult cell; **only after** full vanish start travel to dest / finish travel+fade; chain = sequential land→overlay→travel→next; broken holds 300+300 then vanish with no travel. Enqueue must not silent-skip when piece attribution is weak. When the catapult queue takes over, **cancel own `moveAnimating`** (clear timer / flag) so land→overlay→fling owns the piece — do not let the ordinary move travel race the synthesized land.
- **Center finish (move or push):** sync may already place the piece on a center cell — paint one frame at `lastKnownBoardCellByKey` via `finishAnimFromByKey`, hold until painted (`nextTick` + forced layout + double `rAF`), then clear so CSS slides onto center + fade (`disappearingKeys` / `FINISH_FADE_MS`) (SC-MOVE-76 / SC-FINISH-01/17/18 / D10–D11). Track unfinished board cells with a sync-flush watcher (skip recording center coords; drop any stale `finishAnimFrom` for still-unfinished keys). **Own move onto center:** in `submitMove`, seed only `lastKnownBoardCellByKey` with the piece’s current cell **before** `sendMove` (do **not** pre-seed `finishAnimFromByKey` — silent reject would pin the piece). **Own push onto center:** after successful `sendPush`, seed target `lastKnown` from `push.targetRow/Col` the same way. Finish watch sets `finishAnimFrom` when finished arrives (SC-FINISH-18). Remote finish (other’s move/push) still uses the watch + lastKnown path. **Catapult→center:** defer finish travel until catapult vanish (same pin→travel path).
...
- Drop a new holding grille overlay while the catapult hop queue is still presenting — defer via `pendingGrilleDropKeys` and flush on queue idle (SC-BOARD-29/30).
