...
Sync-driven markers in sticky bottom `.game-hud` (SC-PRESENCE-01…24 / redesign-game-hud). Page reads mirrored `game.seats` + own `steps`/`peeks`/`budgetsInfinite` only — no Colyseus I/O here (`peekedThisTurn` is legacy store mirror, unused by page). **No top-row presence.** Match status strings live in `App.vue` header on Game (SC-PRESENCE-23), not on the page.
...
| Stack | **Always above** avatar (`.say-bubbles--bottom` only). Newer closer to avatar. **No** below-avatar / `.say-bubbles--top` branch; **no** left/right side-slot stacks (SC-SAY-11/12) |
...
- Use Quasar Notify / screen-edge toasts as the say carrier; use `.say-bubbles--top` (below-avatar) stacks.
