...
description: >-
  ...
  (my-moderation / cascadeGap* / hasLive confirm / simplify ACL SC-PACK-100…193 /
  ...
  (crumbs **outside** elevated header SC-BRAND-17) in `src/__tests__/` /
...
| Pinia stores (`auth` / `theme` / `game` / `support` / `content` / `maps`) | [stores.md](stores.md) |
