...
description: >-
  ...
  user-facing copy in pages/components — incl. auth password policy / strength /
  displayName / change-password keys, lobby grilleDensity + catapultDensity
  ...
  favorites `filter*` / `favorite*` / `statusDraft` / `emptyFiltered` — published/
  `in_catalog` list rows show **no** badge SC-PACK-185; `statusInCatalog` key may
  remain unused for list chrome; take/release `takeModeration*` / `moderationTaken` /
  ...
  slot chips / `taskSetLabelFrom` / `back` (editor only — live uses crumbs);
  delete-card confirms; cascade; soft-unpublish confirms SC-PACK-139;
  `content.errors.*` incl. `author_request_open` / `moderation_taken` /
  `moderation_take_required` / `edit_locked` / `pack_unpublished` /
  `last_published_task_set` — RU under en-US; support topic `change_pack`;
  `header.*` (lobby/packs/maps/support/moderation/menu/theme SC-BRAND-11…19);
  content maps `maps.*` (filters/statuses — published rows no badge SC-MAP-45;
...
Boot and message catalog exist; Login / Lobby still mostly **hardcode Russian**. Scaffold keys (`failed` / `success`) are barely used. **Exceptions already on i18n:** `login.google`; **auth-email** `auth.forgot*` / `auth.confirmSentDialog` / cabinet / verify-reminder keys (RU; post-send texts mention папка «Спам»); **password / cabinet profile** `auth.passwordPolicy` / `auth.passwordRule*` / `auth.passwordStrength*` / `auth.displayName*` / `auth.changePassword*` / `auth.currentPassword*` / `auth.logout` (RU; register/reset/change-password); `game.say.*` (preset labels / affordance — never put display copy in the wire `presetId`); `game.readyButton` / `game.countdownSoon`; leave UX `game.leave` (aria for Game brand-logo exit **and** visible label on right-side Game leave control SC-LEAVE-09) / `game.leaveConfirm` / `game.leaveCancel` / `game.leaveExit`; `header.*` (lobby/packs/maps/support/moderation/menu/theme — shared App section nav SC-BRAND-11…19); `auth.backToLobby` (aria-only for non-Game always-button logo → lobby / lobby noop; no page «В лобби» buttons); finish UX `game.finishPlaceModal` / `game.finishPlaceModalOk` / `game.finishStripAria` / `game.finishPlaceBadgeAria`; solo end UX `game.timeExpiredModal` / `game.timeExpiredModalOk` (timer) + `game.stepsExhaustedModal` / `game.stepsExhaustedModalOk` (steps loss — SC-PRESENCE-21); budgets/peek UX `game.stepsCounterAria` / `game.peeksCounterAria` / `game.budgetInfinity` / `game.endTurn` (aria-only for icon `skip_next` on own avatar — no visible label) / `game.peekModal` / `game.peekCorrect` / `game.peekWrong` / `game.peekAffordance` / `game.soloUnlimitedModal` / `game.soloUnlimitedModalOk` (peeks∞ / steps finite); lobby grille density `lobby.grilleDensity` / `lobby.grilleDensityFew` / `lobby.grilleDensityMedium` / `lobby.grilleDensityMany` (мало/средне/много); lobby catapult density `lobby.catapultDensity` / `lobby.catapultDensityFew` / `lobby.catapultDensityMedium` / `lobby.catapultDensityMany` (мало/средне/много); grille UX `game.rescueAffordance` / `game.pushAffordance` / `game.returnAffordance` / `game.allJailWarningModal` / `game.allJailWarningModalOk` (**no** `returnConfirm*` — return uses strip icon only); **support** `support.*` (topics incl. `change_pack` / statuses/guest warning / staff+admin nav / rate-limit `support.errors.*` / roles — RU); **content packs** `content.*` (unified packs list + filters/favorites; **no** in_catalog list badge SC-PACK-185; author re-edit / cancel→draft; `taskSetStatusMarks.*` on live set rows for set author+staff SC-PACK-171…174; crumbs replace «К наборам»/live «Вернуться»; staff «Модерация» in App header not list chrome; «Набор карточек» working copy + unified submit; `addTaskSet*` **no** collection gate; staff soft-unpublish `unpublish` / `republish` / `unpublishedByStaff` / `unpublishConfirm` (warns open requests cancelled — SC-PACK-139); `taskSetLabelFrom`; `statuses.needs_revision` = «Нужна доработка»; `moderationThread` / `reply` / `sendReply` / `emptyThread` / `authorStaff` / `authorYou` / `authorUser`; `slotEmpty` / `slotFilled`; `deleteCardConfirm` / `deleteCardConfirmPublished` by hasLive; quiet autosave; cascade yellow; author delete unpublished confirms; staff Approve / needs-revision / take; **no block UI**; gate login/verify; `content.errors.*` incl. `edit_locked` / `submit_need_task_sets` / `pack_unpublished` — RU). **maps** `maps.*` (list filters/statuses without in_catalog badge; View vs Edit; never-published→Edit; crumbs; `seatConfig` / `viewOnlySubtitle` / paint tools hidden in View). Product copy lives under locale key **`en-US`** (Russian strings) — there is no separate `ru-RU` catalog. Do **not** reintroduce `game.touristChipAria` (chip/`q-menu` removed).

**Auth-email language canon:** new human-facing strings in the auth-email contour (confirm/forgot success, confirm/reset SPA outcomes, cabinet, verify reminder, **password policy / strength / displayName / change-password**) MUST be **Russian**. After any mail send, remind the user to check the spam folder («Спам»). Forgot success must **not** hedge with «если аккаунт существует»; unknown email → `auth.forgotNotFound`. Do not require English copy for these surfaces.
...
export default {
  ...
  auth: {
    forgotSuccess:
      'Письмо со ссылкой отправлено. Проверьте почту и папку «Спам».',
    forgotNotFound: 'Аккаунт с таким email не найден',
    confirmSentDialog: 'Письмо отправлено. Проверьте почту и папку «Спам».',
    passwordPolicy:
      'Пароль: минимум 8 символов, латиница (строчная и заглавная), цифра и символ',
    passwordRuleMin: 'Не меньше 8 символов',
    // … passwordStrength* / displayName* / changePassword* / logout — human-facing RU
