...
description: >-
  ...
  dual submit; `myModeration*` author nav; three-phase `taskSetStatusMarks.*`
  (page subtitle = list marks; `statusCycle*` kept as aliases; `published` /
  `approved`) — ожидает отправки / на модерации / нужна доработка / Опубликовано /
  Одобрено; `unpublishedByStaff`; `draftStaleBanner` / `draftStalePull` /
  `draftStalePullConfirm`; `unpublish` / `republish` / `unpublishTaskSet` + confirms;
  `deleteCardConfirm` / `deleteCardConfirmPublished` by
  hasLive; statuses/threads; quiet autosave; D1′/D5′ locks; collection trash
  remove confirm + author delete
  unpublished; live Edit/`inCollection` copy if any; staff queue
  `staffQueueSubtitle` + tasks-only + rejected label +
  `tasksOnlyHubHint` / no answers approve when tasks-only; nested redirect /
  no block UI; `content.errors.*` incl. `not_approvable` / `pack_unpublished` — RU under en-US).
---
