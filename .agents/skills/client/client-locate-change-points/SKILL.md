...
| Content packs (collection-first; «Набор карточек» + nested tasks; dual submit; statuses/threads; D5′ lock; staff task-set nested) | `stores/content.ts`: HTTP `/api/content/*` via `client.http`; `submitAnswers`/`submitTasks` + `answersDirty`/`tasksDirty` + statuses/`needsModeration`; map API codes with `contentErrorI18nKey`; pages `Content*`; create/edit need verified non-anonymous (page modal; server enforces) |
...
- Content packs collection / catalog / create / «Набор карточек» editor / task-set / moderation / staff nested → `pages/Content*.vue` + `stores/content.ts` (+ verify gate / `auth.isStaff`).
...
| Content packs (collection-first; «Набор карточек» + nested tasks; dual submit; statuses/threads; staff nested) | `pages/Content*.vue` + `stores/content.ts` |
