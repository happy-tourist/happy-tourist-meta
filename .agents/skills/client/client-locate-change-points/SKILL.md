...
| `/content/packs` | `content-catalog` | `ContentCatalogPage`; unified list filters/statuses/star; staff soft-unpublish; `meta.requiresAuth` |
| `/content/collection` | `content-collection` | **redirect** → `content-catalog` |
| `/content/my-moderation` | `content-my-moderation` | `ContentMyModerationPage`; deep-link retained; no non-staff nav; `meta.requiresAuth` |
...
| Global shell / brand logo / section nav / breadcrumbs / theme / leave | `src/App.vue` (elevated `q-header`: logo ≥60px; Packs/Maps/Support + staff Модерация; narrow burger **rightmost** + Acc/Theme/Logout in menu SC-BRAND-19; wide account+theme+session logout; Game status + `header-game-leave`; `app-breadcrumbs` **inside** `q-page-container` SC-BRAND-17…18; `theme.error` banner; logo leave/toLobby/noop; syncs `auth` → `theme`) |
...
| Content packs (unified list + favorites + working copy / author re-edit `editorKind` + `submitPack` + add-task-set **no** collection gate + staff take/lock/save + soft-unpublish/`inCatalog` + cascade + set marks without fan-out + neverLive ghost + list `openRequestType` + no moderation `catalogNav` SC-PACK-126…195 / no block UI) | `stores/content.ts`: HTTP `/api/content/*` (`listCatalog` + `openRequestType`, `starPack`/`unstarPack`, `takeModerationRequest`/`releaseModerationRequest`, `submitPack`, add-task-set, `acquireEditLock`/`staffSavePack`, soft-unpublish, `listMyModeration`; `editorKind`; `TaskSet.neverLive`; `moderationTakeHeldBy`); pages `Content*` + `ContentCatalogPage`; shared queue type `map` |
| Content maps (list filters/statuses + paint editor + author re-edit + staff lock gated by `authorRequestOpen` + soft-unpublish; no collection; SC-MAP) | `stores/maps.ts` + `MapsListPage` / `MapEditorPage` + `MapGridPreview`; take via `content` store |
...
3. Walk ownership:
   ...
   - Colyseus calls only inside `stores/auth`, `stores/theme`, `stores/game`, `stores/support`, `stores/content`, or `stores/maps` (flag page-level `client.*` as a smell to relocate);
