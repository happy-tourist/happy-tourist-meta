...
| Content packs (working copy + unified `submitPack` + add-task-set + staff lock/save + soft-unpublish pack+set/`inCatalog` + cascade yellow + slot chips + add-task-set thread + live drill-in SC-PACK-126…136 + needs_revision / trash/`inCollection` / no block UI) | `stores/content.ts`: HTTP `/api/content/*` via `client.http` (`listMyModeration`, `submitPack`, add-task-set, `acquireEditLock`/`staffSavePack`, `unpublishPack`/`republishPack`, `unpublishTaskSet`/`republishTaskSet`, `needsRevisionRequest`/`cancelRequest`, `loadModeration`/`postModerationMessage`, `deleteUnpublishedPack`/`deleteTaskSet`; `cascadeGapTaskIds`); live `inCollection` + `inCatalog`; map API codes with `contentErrorI18nKey` (incl. `pack_unpublished`, `last_published_task_set`); pages `Content*` (incl. `ContentPackAddTaskSetPage`); create/edit need verified non-anonymous; shared queue also carries type `map` rows |
| Content maps (list/create/paint editor + submit + staff lock/save + soft-unpublish cascade-cancel + author delete never-published; no collection; SC-MAP) | `stores/maps.ts`: HTTP `/api/content/maps*` + `POST /api/content/map/delete` via `client.http`; paint helpers (`paintCell` / `countStarts`); `mapsErrorI18nKey` → `maps.errors.*`; pages `MapsListPage` / `MapEditorPage` + `components/MapGridPreview.vue`; staff hub map branch on `ContentStaff*` / my-moderation via `content` store type `map` |
...
- Content packs collection (Edit unpublished creator/staff; soft-unpublished gray; **no** row add-task-set; trash/click isolation) / catalog (staff unpublish/republish; «На модерации» hidden for staff) / create / author «На модерации» / live (staff Edit+lock + unpublish/republish; add-task-set beside «Задания» in-catalog only; live slot chips) / working-copy editor (`submitPack` / staff save session; Editor `cascade-gap-outline` CSS) / add-task-set page (slot chips + open-request status/thread/reply) / nested tasks / moderation / staff hub (Approve / needs-revision; slot chips; no block UI; **map type badge** + map preview branch) → `pages/Content*.vue` + `stores/content.ts` (+ verify gate / `auth.isStaff`).
- Content maps list / paint editor / staff Edit `?staff=1` → `pages/MapsListPage.vue` / `MapEditorPage.vue` + `components/MapGridPreview.vue` + `stores/maps.ts` (+ lobby «Карты»; no collection; not wired to room create).
...
| Lobby (list / create / join busy-lock; clear stale rooms; quiet resubscribe; Support + «Наборы»→collection + «Карты»→`content-maps`) | `pages/LobbyPage.vue` + `stores/game` `subscribeLobby` / `createGame` / `joinGame` |
...
| Content maps (paint editor; shared staff queue type `map`; soft-unpublish; no collection; SC-MAP) | `pages/MapsListPage.vue` / `MapEditorPage.vue` + `stores/maps.ts` + `MapGridPreview.vue` + `ContentMaps.test.ts` |
