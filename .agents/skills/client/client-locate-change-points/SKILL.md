...
| Content packs (unified list + favorites + working copy / author re-edit `editorKind` + `submitPack` + add-task-set **no** collection gate + staff take/lock/save + soft-unpublish/`inCatalog` + cascade SC-PACK-126…166 / no block UI) | `stores/content.ts`: HTTP `/api/content/*` (`listCatalog`, `starPack`/`unstarPack`, `takeModerationRequest`/`releaseModerationRequest`, `submitPack`, add-task-set, `acquireEditLock`/`staffSavePack`, soft-unpublish, `listMyModeration`; `editorKind`; `moderationTakeHeldBy`); pages `Content*` + `ContentCatalogPage`; shared queue type `map` |
