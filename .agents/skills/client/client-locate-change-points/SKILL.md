...
| Global shell / brand logo / section nav / breadcrumbs / theme / leave | `src/App.vue` (elevated `q-header`: logo ≥60px; Packs/Maps/Support + staff Модерация; burger narrow; account+theme+session logout off-Game; Game status + `header-game-leave`; page-zone `app-breadcrumbs` **below** header SC-BRAND-17; `theme.error` banner; logo leave/toLobby/noop; syncs `auth` → `theme`) |
