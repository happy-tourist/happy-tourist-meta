...
| `/content/packs/:id/edit` | `content-pack-edit` | `ContentPackEditorPage` (answers); `meta.requiresAuth` |
| `/content/packs/:id/tasks/:taskSetId` | `content-pack-tasks` | `ContentPackTasksPage`; `meta.requiresAuth` |
...
| `/content/staff/requests/:id` | `content-staff-request` | `ContentStaffRequestPage`; answers hub; `requiresAuth` + `requiresStaff` |
...
| Content packs (collection-first; answers + nested tasks; dual submit; staff answers hub) | `stores/content.ts`: HTTP `/api/content/*` via `client.http`; `submitAnswers`/`submitTasks` + `answersDirty`; map API codes with `contentErrorI18nKey`; pages `Content*`; create/edit need verified non-anonymous (page modal; server enforces) |
...
- Content packs collection / catalog / create / answers editor / task-set / moderation / staff → `pages/Content*.vue` + `stores/content.ts` (+ verify gate / `auth.isStaff`).
...
| Content packs (collection-first; answers + nested tasks; dual submit; staff answers hub) | `pages/Content*.vue` + `stores/content.ts` |
