...
description: >-
  ...
  content working-copy editor + add-task-set page + staff Edit lock + unified submit / staff save + author «На модерации» + cascade/hasLive confirm + `cascade-gap-outline` + collection trash/click isolation (no row `:to`) + author delete unpublished + staff queue pending\|needs_revision / no block UI, App brand logo (always-button ≥60px; Game leave / auth+other → lobby / lobby noop;
...
| `/content/collection` | `content-collection` | `ContentCollectionPage` | `meta.requiresAuth`; own collection (Edit unpublished creator **or** staff; published non-staff → add-task-set; trash/`delete` + confirm; hide Edit if `blocked`; **no** row `:to`) |
| `/content/my-moderation` | `content-my-moderation` | `ContentMyModerationPage` | `meta.requiresAuth`; author «На модерации» (`listMyModeration`; pending\|needs_revision → Edit) |
...
| `/content/packs/:id` | `content-pack` | `ContentPackPage` | `meta.requiresAuth`; live view (staff Edit + lock; non-staff add-task-set when verified+inCollection; collect from `inCollection`) |
| `/content/packs/:id/edit` | `content-pack-edit` | `ContentPackEditorPage` | `meta.requiresAuth`; creator working copy (`submitPack`) or staff live/working edit (`staffSavePack` + lock; no Submit) |
...
| `/content/packs/:id/tasks/:taskSetId` | `content-pack-tasks` | `ContentPackTasksPage` | `meta.requiresAuth`; nested task-set (unpublished / staff) |
...
| Content packs | `ContentCatalogPage` / `ContentCollectionPage` (Edit unpublished creator/staff; add-task-set on published; trash; no row `:to`) / `ContentMyModerationPage` / `ContentPackPage` (staff Edit+lock; non-staff add-task-set) / `ContentPackCreatePage` / `ContentPackEditorPage` (working copy `submitPack` / staff `staffSavePack`) / `ContentPackAddTaskSetPage` / `ContentPackTasksPage` / `ContentPackModerationPage` / `ContentStaffPage` / `ContentStaffRequestPage` (Approve / needs-revision; no block UI) / `ContentStaffTasksPage` (→ hub) | `stores/content` HTTP; working copy + unified submit + add-task-set + staff lock; cascadeGap*; `requiresAuth`; create/edit need verified non-anonymous; staff `requiresStaff` |
