...
description: >-
  ...
  strip HUD row/2×2, budgets/end-turn dock, return confirm modal, nearest-center
  finish, grille trap/rescue) in this Quasar Vue 3 client.
---
...
| Game session | `GamePage` | `stores/game` `rejoinGame`/`sendMove`/`sendRescue`/`sendReturnFromFinish`/`sendPeek`/`sendPeekAnswer`/`sendEndTurn`/`sendSay`; top presence + seated sticky `.game-hud` (own + strip row/2×2; no chip/`q-menu`); unfinished pieces + holes + grille overlays (`GRILLE_ANIM_MS=1000`) + trap/rescue + return modal + all-jail modal + budgets beside avatar / end-turn dock + peek + finish nearest-center / timeout UX + dual rings + say top↓ / own↑; route param `roomId` (reconnect only — **not** shown in chrome); `meta.requiresAuth` |

Room name `tourist` + live lobby align with `../happy-tourist-server`; Game mirrors seats/`finishPlace`/`timeExpired`/piece `finished`/`trapped`/`currentTurnSessionId`/`turnUntil`/`turnBudgetSeconds`/`removedTaskKeys`/`holdingGrilleKeys`, listens private `budgets`/`peekOpen`/`allJailWarning`, and renders unfinished pieces (after materialize) + holes + grille overlays + rescue/return-modal chrome + own counters (above avatar) / end-turn dock + peek chrome + finish/timeout + top + seated-bottom presence rings + strip + local move chrome (eligible seats; move does not end turn; trapped locked) + ephemeral say bubbles (top↓ / own↑).
