...
description: >-
  ...
  content «Набор карточек» + live Edit when `inCollection` (+ pending-author exception; hide if `blocked`/foreign pending) + collection trash/`delete` + click isolation (no row `:to`) + author delete unpublished + D1′ lock + staff queue tasks-only + same hub / no answers approve when tasks-only / no block UI, App brand logo (always-button ≥60px; Game leave / auth+other → lobby / lobby noop;
...
| `/content/collection` | `content-collection` | `ContentCollectionPage` | `meta.requiresAuth`; own collection (lobby entry; Edit + trash/`delete` + confirm; **no** row `:to` — `@click` navigate + `@click.stop` on side icons; hide Edit if `blocked`; row→live/`hasLive` else editor) |
...
| `/content/staff` | `content-staff` | `ContentStaffPage` | `meta.requiresAuth` + `requiresStaff`; answers + tasks-only queue |
| `/content/staff/requests/:id` | `content-staff-request` | `ContentStaffRequestPage` | `meta.requiresAuth` + `requiresStaff`; same hub layout; hide answers actions when `tasksOnly` / `answersActionsAvailable === false`; no block UI |
...
| Content packs | `ContentCatalogPage` / `ContentCollectionPage` (Edit + trash + confirm; no row `:to`; hide Edit if `blocked`) / `ContentPackPage` (live Edit when `inCollection` + pending-author; hide if `blocked`/foreign pending; collect from `inCollection`; verify gate) / `ContentPackCreatePage` / `ContentPackEditorPage` («Набор карточек» + author delete unpublished) / `ContentPackTasksPage` (D1′ lock + delete set) / `ContentPackModerationPage` / `ContentStaffPage` (answers + tasks-only queue) / `ContentStaffRequestPage` (same hub; hide answers actions when tasks-only; no block UI) / `ContentStaffTasksPage` (→ hub, or queue if tasks-only) | `stores/content` HTTP; dual submit; D1′/D5′ locks; live `inCollection` + pending author ids; `tasksOnly`/`answersActionsAvailable`; `requiresAuth`; create/edit need verified non-anonymous; staff `requiresStaff` |
