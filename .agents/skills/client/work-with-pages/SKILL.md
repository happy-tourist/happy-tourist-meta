...
- `SupportPage` — create ticket + own list via `useSupportStore()`; guest banner (no email notify + session-loss risk); after success clear fields + `resetValidation`.
- `SupportTicketPage` — public thread (visible gap between messages); author reply/close; staff take / set-awaiting (**only when** `status === 'in_progress'`) / close; reply clear + `resetValidation`; closed read-only + CTA new ticket.
- `SupportStaffPage` — staff queue (`requiresStaff`); topic filter (default all) + status `open`\|`closed`\|`all` (default `open`); refresh list.
- `AdminUsersPage` — list users + change role (`requiresAdmin`); server excludes anonymous; badge/chip when `emailVerified === false`; moderator has no role UI.
