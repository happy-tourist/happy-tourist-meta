...
description: >-
  ...
  and game (top opponents presence, seated
...
- `AccountPage` — cabinet: displayName form; change-password if `canChangePassword`; confirm send + change email; logout; soft-verify (no hard-gate); anonymous redirect to lobby on mount.
...
- `SupportPage` — create ticket + own list via `useSupportStore()`; guest banner (no email notify + session-loss risk); create form: `lazy-rules` + clear → `await nextTick()` → `resetValidation` (see `work-with-forms`).
