...
| Lobby / rooms | `LobbyPage` | `stores/game.subscribeLobby`, create `{ mapId, packId, taskSetIds, maxSeats, grilleDensity, catapultDensity }` / join; **no** page section/logout chrome (App header → `content-catalog` / `content-maps` / support); `meta.requiresAuth` |
