...
description: >-
  ...
  content «Набор карточек» + author «На модерации» + three-phase marks + cascade/hasLive confirm + `cascade-gap-outline` (not `bg-warning`) + task-row slots + live/staff slot chips + subtitle=`taskSetStatusMarks.*` (incl. published/approved) + stale-draft banner+pull + live Edit when `inCollection` (+ open-author exception; hide if `blocked`/foreign open) + collection/catalog nav → my-moderation + trash/`delete` + click isolation (no row `:to`) + staff unpublish/republish + hide Edit for non-staff after staff-unpublish + staff task-set unpublish when ≥2 live + author delete unpublished + D1′/D5′ on open (pending|rejected) + staff queue tasks-only + rejected («нужна доработка»; approve-from-rejected) + same hub / no answers approve when tasks-only / no block UI, App brand logo (always-button ≥60px; Game leave / auth+other → lobby / lobby noop;
...
| `/content/collection` | `content-collection` | `ContentCollectionPage` | `meta.requiresAuth`; own collection (lobby entry; nav → catalog + «На модерации»; Edit + trash/`delete` + confirm; staff unpublish/republish next to Edit; badge `unpublishedByStaff` ≠ `draftOnly`; hide Edit if `blocked` or (non-staff ∧ `unpublishedByStaff`); **no** row `:to` — `@click` navigate + `@click.stop` on side icons; row→live/`hasLive` else editor) |
...
| `/content/packs/:id` | `content-pack` | `ContentPackPage` | `meta.requiresAuth`; live view (Edit when `inCollection` + pending-author exception; hide if `blocked`/foreign pending; collect from `inCollection`; slot chips not bare `slotsCount`; verify gate on Edit) |
| `/content/packs/:id/edit` | `content-pack-edit` | `ContentPackEditorPage` | `meta.requiresAuth`; «Набор карточек» (three-phase marks + thread; delete-card confirm by `hasLive`; cascade save no client pre-clear; `cascade-gap-outline` on gaps; `draftStale` banner + pull/`rebaseDraft`; staff task-set unpublish when ≥2 live; quiet autosave; author delete unpublished) |
| `/content/packs/:id/tasks/:taskSetId` | `content-pack-tasks` | `ContentPackTasksPage` | `meta.requiresAuth`; nested task-set (D1′/D5′ locks; `cascade-gap-outline` on `cascadeGap*`; task-row slots; subtitle=`taskSetStatusMarks.*` incl. published/approved; `draftStale` banner+pull; author delete set) |
...
| `/content/staff/requests/:id/tasks` | `content-staff-request-tasks` | `ContentStaffTasksPage` | `meta.requiresAuth` + `requiresStaff`; nested tasks → hub after approve (queue if tasks-only); slot chips; no `pack_not_public` dead-end |
...
| Content packs | `ContentCatalogPage` / `ContentCollectionPage` (nav → «На модерации»; Edit + trash + confirm; staff unpublish/republish; hide Edit if `blocked` or non-staff after staff-unpublish; no row `:to`) / `ContentMyModerationPage` (author «На модерации») / `ContentPackPage` (live Edit when `inCollection` + open-author; hide if `blocked`/foreign open; collect from `inCollection`; slot chips; verify gate) / `ContentPackCreatePage` / `ContentPackEditorPage` («Набор карточек» + three-phase + hasLive delete confirm + cascade outline + draftStale pull + staff task-set unpublish + author delete) / `ContentPackTasksPage` (D1′/D5′ on open + cascade outline + task-row slots + subtitle=`taskSetStatusMarks.*` + draftStale + delete set) / `ContentPackModerationPage` / `ContentStaffPage` (answers + tasks-only + rejected queue; approve-from-rejected; `queueStatusLabel`) / `ContentStaffRequestPage` (same hub; hide answers actions when tasks-only; no block UI) / `ContentStaffTasksPage` (→ hub, or queue if tasks-only; slot chips) | `stores/content` HTTP; dual submit; my-moderation; cascadeGap*; draftStale/rebase; unpublish/republish; D1′/D5′ on open=pending\|rejected; three-phase + published/approved; live `inCollection` + `unpublishedByStaff` + open author ids; staff pending\|rejected/`tasksOnly`; `requiresAuth`; create/edit need verified non-anonymous; staff `requiresStaff` |
