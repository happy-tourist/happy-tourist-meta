...
description: >-
  ...
  content working-copy editor + add-task-set page (beside «Задания», not header/collection row) + staff Edit lock session cards↔tasks (`isStaffEditSessionNavigation`) + unified submit / staff save + staff soft-unpublish/republish (`inCatalog`) + author «На модерации» (hidden for staff) + stable tooltip/reserved hints (no jumping captions) + cascade/hasLive confirm + `cascade-gap-outline` on Editor **and** Tasks (SC-PACK-126) + slot chips on all question lists (SC-PACK-127) + add-task-set status/thread/reply (SC-PACK-128) + collection trash/click isolation (no row `:to`; soft-unpublished gray) + author delete unpublished + staff queue pending\|needs_revision / no block UI, App brand logo (always-button ≥60px; Game leave / auth+other → lobby / lobby noop;
...
| `/content/collection` | `content-collection` | `ContentCollectionPage` | `meta.requiresAuth`; own collection (Edit unpublished creator **or** staff; **no** row add-task-set; trash/`delete` + confirm; soft-unpublished gray non-nav for non-staff; hide Edit if `blocked`; **no** row `:to`) |
...
| Content packs | `ContentCatalogPage` (staff unpublish/republish) / `ContentCollectionPage` (Edit unpublished creator/staff; soft-unpublished gray; **no** row add-task-set; trash; no row `:to`) / `ContentMyModerationPage` / `ContentPackPage` (staff Edit+lock; unpublish/republish; add-task-set beside «Задания»; live slot chips) / `ContentPackCreatePage` / `ContentPackEditorPage` (working copy `submitPack` / staff `staffSavePack` session; **cascade-gap-outline CSS** on task-set rows) / `ContentPackAddTaskSetPage` (slot chips + open-request status/thread/reply) / `ContentPackTasksPage` (task yellow + slots) / `ContentPackModerationPage` / `ContentStaffPage` / `ContentStaffRequestPage` (Approve / needs-revision; **slot chips** on question rows; no block UI) / `ContentStaffTasksPage` (→ hub) | `stores/content` HTTP; working copy + unified submit + add-task-set + staff lock session + soft-unpublish (`inCatalog`); cascadeGap*; SC-PACK-126…128; `requiresAuth`; create/edit need verified non-anonymous; staff `requiresStaff` |
