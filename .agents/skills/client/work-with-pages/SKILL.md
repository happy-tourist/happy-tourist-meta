...
description: >-
  ...
  content «Набор карточек» + author «На модерации» + three-phase marks + live Edit when `inCollection` (+ open-author exception; hide if `blocked`/foreign open) + collection/catalog nav → my-moderation + trash/`delete` + click isolation (no row `:to`) + author delete unpublished + D1′/D5′ on open (pending|rejected) + staff queue tasks-only + rejected («нужна доработка»; approve-from-rejected) + same hub / no answers approve when tasks-only / no block UI, App brand logo (always-button ≥60px; Game leave / auth+other → lobby / lobby noop;
...
| `/content/packs/:id/edit` | `content-pack-edit` | `ContentPackEditorPage` | `meta.requiresAuth`; «Набор карточек» (three-phase marks + thread; quiet autosave; author delete unpublished) |
| `/content/packs/:id/tasks/:taskSetId` | `content-pack-tasks` | `ContentPackTasksPage` | `meta.requiresAuth`; nested task-set (D1′/D5′ locks; three-phase marks; author delete set) |
...
| Content packs | `ContentCatalogPage` / `ContentCollectionPage` (nav → «На модерации»; Edit + trash + confirm; no row `:to`; hide Edit if `blocked`) / `ContentMyModerationPage` (author «На модерации») / `ContentPackPage` (live Edit when `inCollection` + open-author; hide if `blocked`/foreign open; collect from `inCollection`; verify gate) / `ContentPackCreatePage` / `ContentPackEditorPage` («Набор карточек» + three-phase marks + author delete) / `ContentPackTasksPage` (D1′/D5′ on open + three-phase + delete set) / `ContentPackModerationPage` / `ContentStaffPage` (answers + tasks-only + rejected queue; approve-from-rejected; `queueStatusLabel`) / `ContentStaffRequestPage` (same hub; hide answers actions when tasks-only; no block UI) / `ContentStaffTasksPage` (→ hub, or queue if tasks-only) | `stores/content` HTTP; dual submit; my-moderation; D1′/D5′ on open=pending\|rejected; three-phase marks; live `inCollection` + open author ids; staff pending\|rejected/`tasksOnly`; `requiresAuth`; create/edit need verified non-anonymous; staff `requiresStaff` |
