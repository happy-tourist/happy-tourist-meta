...
|-------|------|-------------------|
| **auth** | `user` (`emailVerified?`, optional `theme`), `token`, `loading`, `error`, `ready`; `isAuthenticated`, `displayName`, `needsEmailVerification`; register/login/anonymous/Google/`logout`/`forgotPassword`/`sendEmailConfirmation`/`changeEmail`/`refreshUserData`/`whenReady` | `LoginPage`, `ForgotPasswordPage`, `AccountPage`, router `beforeEach`, `LobbyPage` logout/cabinet, `App.vue` theme sync + verify reminder |
...
- **auth** owns Colyseus Auth only (`client.auth.*`, thin `client.http` for send-confirm / change-email, token sync via `onChange`). It does not create rooms or call Dark/`GET|POST /api/theme`.
