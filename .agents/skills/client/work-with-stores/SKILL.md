...
src/stores/
  ...
  content.ts        # setup store — packs catalog / collection (confirm remove) / draft; dual submit; D1′/D5′ locks; author delete unpublished; staff hub redirect; no block UI
...
| `content` | **Setup** (`defineStore('content', () => { … })`) | HTTP packs via `client.http` (catalog/collection confirm-remove/live/draft/`submitAnswers`/`submitTasks`/author delete unpublished/moderation/staff); draft statuses + requestId; `answersDirty`/`tasksDirty` + `needsModeration` + pending per type; **D1′** + D5′ locks; staff hub omit moderated / post-approve redirect; no block UI; `loading` / `error` + page `q-banner` |
...
| **content** | catalog / collection (confirm remove) / live / draft / `submitAnswers` / `submitTasks` / author delete unpublished pack\|task-set / moderation + staff hub (actionable list → nested → redirect); D1′/D5′ locks; `loading` / `error` via `client.http`; map codes with `contentErrorI18nKey` | `ContentCatalogPage`, `ContentCollectionPage`, `ContentPackPage`, `ContentPackCreatePage`, `ContentPackEditorPage`, `ContentPackTasksPage`, `ContentPackModerationPage`, `ContentStaffPage`, `ContentStaffRequestPage`, `ContentStaffTasksPage` |
...
- Call `submitAnswers` / `submitTasks` (and mirror draft flags `answersDirty`/`tasksDirty` / statuses / `needsModeration` / pending per type / **D1′** + D5′ locks); never invent a single `/submit` client path; pages: quiet autosave + submit disabled without dirty/minima; collection-only Edit; confirm remove; author delete unpublished with confirm; staff no block UI.
