...
- `GamePage.vue`: `LAYOUT` tiles + piece assets; presence layout / grace tick / `consentedLeaving`; flatten `seat.pieces` for board overlay; strip×4 from `mySeat`; local `selectedSide` / `moveAnimating` / legal hints; say picker open state (not Pinia).
