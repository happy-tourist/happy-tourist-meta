...
src/stores/
  ...
  content.ts        # setup store — packs catalog / collection (trash confirm) / live (`inCollection` + `pendingAnswersAuthorId`/`pendingTasksAuthorId`; patch `inCollection` on add/remove) / draft; dual submit; D1′/D5′ locks; author delete unpublished; staff hub redirect; no block UI
