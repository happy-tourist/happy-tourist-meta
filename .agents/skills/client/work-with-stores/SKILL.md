...
description: >-
  ...
  removed-task holes, grille trap/rescue/push/return, peek/end-turn), acceptHMRUpdate,
...
- **auth** owns Colyseus Auth only (`client.auth.*`, thin `client.http` for send-confirm / change-email / confirm-email / reset-password, token sync via `onChange`). It does not create rooms or call Dark/`GET|POST /api/theme`.
...
- Map only needed room fields from `onStateChange` (incl. `currentTurnSessionId` / `removedTaskKeys` / `holdingGrilleKeys` / piece `trapped`); keep `sendMove` / `sendRescue` / `sendPush` / `sendReturnFromFinish` / `sendPeek` / `sendEndTurn` / `sendSay` lockstep with server `onMessage`.
