...
| `client.pages` | `../happy-tourist.github.io/src/pages/` | route pages (Login/Lobby/Support/Content*/Maps*/Admin/Game) |
| `client.stores` | `../happy-tourist.github.io/src/stores/` | Pinia (`auth`, `theme`, `game`, `support`, `content`, `maps`) |
...
| `server.lib` | `../happy-tourist-server/src/lib/` | mailer + support tickets/roles + content packs (`content.ts`) + content maps (`contentMaps.ts`) + passwordPolicy helpers |
...
| `server.db` | `../happy-tourist-server/src/db/` | GameDatabase / users + support + content_* pack + map tables |
