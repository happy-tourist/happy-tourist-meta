...
- **Budgets / end-turn:** steps и peeks **друг под другом справа** от своего аватара; кнопка завершить ход — **над** нижней панелью справа (у нижнего правого края области доски).
- **Top presence gap:** маркеры оппонентов/spectator **вплотную** к доске (без большого пустого зазора под рядом; say bubbles absolute поверх верха доски при необходимости).
...
- Capability ID:
  - `game/presence` — top opponents / spectator top (tight to board); bottom own + strip; budgets справа от аватара; end-turn над панелью справа
...
- `game/presence`: opponents (и spectator all) над доской вплотную; bottom = own + strip; budgets справа от аватара; end-turn над панелью справа; sticky низ и header status/leave без отката
