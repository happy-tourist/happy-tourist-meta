...
#### Scenario [SC-BOARD-09]: Incorrect answer grants nothing
...
- **AND** the peeks budget decreases by 1 in finite mode
- **AND** the task tile is still removed for all clients
...
When a peek resolves (correct, incorrect, or timeout-forced incorrect), the server SHALL mark that task cell as removed in synchronized state visible to every client in the room. Every client MUST render a removed task cell as an empty hole (page background, no brown tile chrome). Removed task cells MUST remain legal landing and standing cells for moves (same playable set as before removal). Pieces already on the cell MUST remain on that cell. Start holes that were never playable stay non-playable.

#### Scenario [SC-BOARD-11]: Everyone sees the hole after peek

- **GIVEN** a peek resolves on a task cell while seated players and a spectator view the board
...
- **GIVEN** a tourist piece stands on a task cell that is then removed by a peek
