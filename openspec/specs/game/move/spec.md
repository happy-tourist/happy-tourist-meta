...
Очередь хода и перемещение фигурок туриста на одну соседнюю клетку: авторитетная валидация на сервере, локальные подсказки только ходящему, плавная анимация у всех клиентов. Вход на center завершает фишку (`game/finish`); finished-seats пропускаются в очереди хода.

Связанные capability: фазы старта и gate ходов — `game/start`; рассадка и pieces — `game/pieces`; геометрия поля — `game/board`; reconnect grace — `game/presence` / pieces; финиш — `game/finish`.
