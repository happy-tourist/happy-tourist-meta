...
Related: seating gate — `game/pieces` (new seats only in `waiting`); return geometry and steps — `game/move`; push finish — `game/move` SC-MOVE-76; catapult fling onto center — `game/move` / `game/board`.
