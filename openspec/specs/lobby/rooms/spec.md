...
Live-список доступных игровых комнат на экране лобби: авторизованный пользователь (включая гостя) видит актуальные комнаты `checkers` в реальном времени и может создать или присоединиться к партии.
...
| SC-LOBBY-04 | covered (registration + tests/loadtest `checkers`, tasks 1.2/1.4) |
...
The system SHALL provide an authorized user on the lobby screen with a live listing of available `checkers` game rooms over a realtime Colyseus connection to the lobby listing room, without periodic HTTP polling of the room list while the lobby screen is active.
...
- **THEN** the client establishes a realtime subscription to the lobby listing filtered to room name `checkers`
- **AND** the UI receives an initial snapshot of available `checkers` rooms
...
- **GIVEN** at least one client is subscribed to the lobby listing for `checkers`
- **WHEN** a new `checkers` room becomes available for listing
...
- **GIVEN** a `checkers` room is visible in the live lobby listing
...
### Requirement: Canonical checkers room name

The server SHALL register the playable game room under the name `checkers`, matching the client contract for create, join, joinOrCreate, and lobby filter.

#### Scenario [SC-LOBBY-04]: Create and join by checkers name

- **GIVEN** the server is running with the game room registered as `checkers`
- **WHEN** an authenticated client creates or joins a game via the lobby actions (create / joinOrCreate / join by id of a listed `checkers` room)
- **THEN** the operation targets the `checkers` room type
- **AND** newly created rooms are eligible for the live lobby listing of `checkers`

### Requirement: Leave lobby listing when entering a game

While the user is on the game screen in an active `checkers` session, the client SHALL NOT keep an active lobby listing subscription. The lobby listing subscription MUST be closed before or when entering a game room, and MAY be re-established when the user returns to the lobby screen.
...
- **WHEN** the user successfully enters a `checkers` game (play / create / join)
...
- **THEN** the client establishes a new lobby listing subscription and shows the current `checkers` room list
